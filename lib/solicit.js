...
const VirtualAgent = require("./handler/virtual");
...
Solicitation.prototype.to_json = function () {
  var data = {
    ...
    contractors: _.map(this.contractors, function (c, username) {
      return {
        rate: c.rate,
        selected: c.selected,
        username: username,
...
Solicitation.prototype.from_json = function (data) {
  ...
  if (data.contractor && _.isFinite(data.rate)) {
    const contractor = new VirtualAgent({
