...
const Solicitation = function (data) {
  ...
  this.applied_bonus_minutes = 0;
  this.remaining_client_bonus_minutes = null;
  ...
  _.each(["username", "id", "path", "preferred_contractor", "description",
    "tags", "total_bill", "cut", "duration", "client_account"],
    function (k) {
      self[k] = data[k];
    });
...
Solicitation.prototype.toString = function () {
  let s = util.format("%s creator %s for %s state %s", this.id, this.username, this.path, SOLICIT_STATES_REVERSE[this.state]);
  if (this.start) {
...
Solicitation.prototype.to_json = function () {
  return {
    ...
    applied_bonus_minutes: this.applied_bonus_minutes,
    remaining_client_bonus_minutes: this.remaining_client_bonus_minutes,
  };
};
...
Solicitation.prototype.complete = function (complete_cb) {
  ...
  const auto = {
    ...
    perms_remove: function (cb) {
      ...
      api_client.perms_set(self.get_selected_contractor(), self.path, [], cb);
    },
    ...
    charge: ["update_state", function (cb) {
      api_client.solicitation_charge(self.id, {}, function (err, result) {
        ...
        self.duration = result.duration;
        self.total_bill = result.total_bill;
        self.applied_bonus_minutes = result.applied_bonus_minutes;
        self.remaining_client_bonus_minutes = result.remaining_client_bonus_minutes;
...
Solicitation.prototype.on_hire = function (agent, data, hire_cb) {
  ...
  var contractor = this.contractors[data.username];
  if (!contractor) {
    log.warn("Direct hire %s", data.username);
  ...
  const auto = {
    ...
    solicitation_hire: ["verify_charge", function (cb) {
      const req = {
        contractor: data.username,
        state: SOLICIT_STATES_REVERSE[SOLICIT_STATES.HIRED],
      };
      if (contractor.rate) {
        req.rate = contractor.rate;
      }
      api_client.solicitation_set(self.id, req, cb);
    }],
    perms_set: ["verify_charge", function (cb) {
      ...
      api_client.perms_set(data.username, self.path, perms_list, cb);
    }]
  };
  ...
  async.auto(auto, function (err, result) {
    if (err) {
      ...
      contractor.rate = result.solicitation_hire.rate;
      try {
...
Solicitation.prototype.on_feedback = function (agent, data, cb) {
  ...
  if (!_.includes([SOLICIT_STATES.ABSOLVED, SOLICIT_STATES.COMPLETED], this.state)) {
