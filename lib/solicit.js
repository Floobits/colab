...
Solicitation.prototype.start_update_timeout = function () {
  ...
  this.last_keep_alive = setTimeout(function () {
    self.enter_state("timed out", SOLICIT_STATES.COMPLETED);
...
Solicitation.prototype.stop_update_timeout = function () {
  if (!this.update_timeout) {
    log.warn("stop_update_timeout: No update_timeout for %s", this.toString());
    return;
  }
...
Solicitation.prototype.enter_state = function (agent_name, state) {
  ...
  if (state < this.state) {
    throw new Error(util.format("Cannot %s from %s state.", state, SOLICIT_STATES_REVERSE[this.state]));
  }
...
Solicitation.prototype.on_keep_alive = function (agent, data, cb) {
  ...
  self.last_keep_alive = setTimeout(function () {
    ...
    self.enter_state(`${agent.username} timed out`, SOLICIT_STATES.COMPLETED);
...
Solicitation.prototype.on_complete = function (agent, data, complete_cb) {
  ...
  async.auto(auto, function (err, result) {
    if (err) {
      ...
      self.enter_state(agent.username, SOLICIT_STATES.COMPLETED);
