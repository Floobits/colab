...
var on_workspaces_all = function (server, req, res) {
  ...
  fs.readdir(settings.buf_storage.local.dir, function (err, files) {
    ...
    _.each(files, function (file) {
      // TODO: build workspace versions in a leveldb or something. this is a total hack
      var active,
        workspace = parseInt(file, 10);
      if (!_.isFinite(workspace)) {
        return;
      }
      active = !!(server.workspaces[file]);
      response[file] = {
        active: active
      };
    });
...
var listen = function (port, server) {
  ...
  app.get("/metrics/:metric:", on_metrics.bind(app, server));
