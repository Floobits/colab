...
Server.prototype.handle_get_workspace_by_path = function (req, res) {
  ...
  if (path_parts.length === 1) {
    db.get_user(owner, function (err, result) {
      if (err) {
        if (result && result.rowCount === 0) {
          return res.send(404);
        }
        log.error("Error finding workspace %s: %s", owner, err);
        return res.send(500, err);
      }
      self.handle_get_workspace({ params: { workspace_id: result.id }}, res, true);
    });
...
Server.prototype.handle_wallops = function (req, res) {
  ...
  async.each(_.values(self.controller.colab_servers), function (colab_server, cb) {
    var options = {
        auth: {
          user: AUTH_USER,
          password: AUTH_PASS
...
Server.prototype.handle_motd = function (req, res) {
  ...
  async.each(_.values(self.controller.colab_servers), function (colab_server, cb) {
    ...
    log.warn("SET MOTD TO", req.body.msg);
    res.send(200, req.body.msg);
  });
};
...
Server.prototype.handle_get_stats = function (req, res) {
  ...
  stats.moved_workspaces = self.controller.moved_workspaces.slice(-100);
