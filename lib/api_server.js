...
var on_workspaces_active = function (server, req, res) {
  ...
  response.workspaces = _.map(server.workspaces, function (workspace) {
    return {
      ...
      users: _.map(workspace.agents, function (agent) {
        return {
          client: agent.client,
...
var on_workspace_get = function (server, req, res) {
  ...
  auto.events = ["events_rs", function (cb, response) {
    ...
    rs.on("data", function (data) {
      ...
      evt = new RoomEvent(evt_id, row.name, row);
      events.push(evt);
    });
  }];
...
var create_workspace = function (server, req, res) {
  var auto = {},
  ...
  async.auto(auto, function (err, result) {
    ...
    if (err) {
      ...
      log.error("Error creating workspace %s: %s", workspace_id, err);
      return res.send(500, err);
    }
    return res.json(201, {
      version: version,
...
var fetch_workspace = function (server, req, res) {
  ...
  auto.del_local_events = ["local_readstream", function (cb) {
  ...
  auto.get_bufs = ["compare_bufs", function (cb, response) {
    ...
    _.each(response.get_buf_list.events, function (evt) {
      evt = new RoomEvent(evt);
      log.debug("Writing room event: %s", JSON.stringify(evt.to_json()));
      ws.write({
        key: util.format("event_%s", evt.id),
        value: JSON.stringify(evt.to_json())
      });
    });
...
var listen = function (port, server) {
  ...
  app.post("/workspace/:workspace_id", auth, create_workspace.bind(app, server));
