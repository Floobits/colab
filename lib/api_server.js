...
var create_workspace = function (server, req, res) {
  ...
  auto.set_version = function (cb) {
    server.db.put(util.format("version_%s", workspace_id), version, cb);
  };
  auto.create_db = function (cb) {
    ldb.get_db(null, workspace_id, { createIfMissing: true }, cb);
  };
  async.auto(auto, function (err, result) {
    if (result.create_db) {
...
var listen = function (port, server) {
  ...
  app.post("/workspace/:workspace_id", create_workspace.bind(app, server));
