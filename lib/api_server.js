...
var on_buf_get = function (server, req, res) {
  ...
  if (workspace) {
    ...
    // TODO: send proper mime types based on file extensions?
    if (buf.encoding === "utf8") {
      // Send plaintext content type for text bufs
      res.type("text/plain");
    }
  ...
  ldb.get(null, workspace_id, util.format("buf_content_%s", buf_id), "binary", function (err, result) {
    ...
    ldb.get(null, workspace_id, util.format("buf_%s", buf_id), "json", function (err, result) {
