...
var Server = function (controller) {
  ...
  self.app.get("/user/:user_id/now_editing", self.handle_get_user_now_editing.bind(self));
...
Server.prototype.handle_get_active_workspace = function (req, res) {
  ...
  let server = this.controller.server_mapping.workspace[workspace_id];
  if (!server) {
    return res.status(404).end();
  }
  let workspace = this.controller.workspaces[workspace_id].slaves[server.id];
