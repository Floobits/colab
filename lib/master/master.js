...
var Master = function () {
  ...
  actions.slave.onUPDATE_LOAD(function (id, load) {
    log.log("Slave %s load %s", id, load);
  });
...
Master.prototype.on_update_count = function (id, workspaces) {
  ...
  _.each(workspaces, function (workspace) {
    ...
    if (workspace.users) {
      w.colabs[id].users = workspace.users;
    }
  });
...
Master.prototype.set_mapping = function (workspace_id, colab) {
  var self = this;
...
Master.prototype.create_workspace = function (workspace_id, atts, cb) {
  var self = this,
    colab,
    options = {
      auth: {
...
Master.prototype.rebalance = function () {
  ...
  colab_servers = _.filter(self.slaves, function (colab) {
...
Master.prototype.replicate = function () {
  ...
  _.each(self.workspaces, function (w, id) {
    ...
    _.each(w.colabs, function (colab, colab_id) {
      var slave = self.slaves[colab_id];
      ...
      if (slave.poller.errors > settings.colab_error_threshold) {
        log.debug("Colab %s poller errors too high: %s", colab_id, slave.poller.errors);
...
Master.prototype.update_memcached_users = function () {
  ...
  _.each(self.slaves, function (colab) {
...
exports.run = function (cb) {
  ...
  //   colabs.push(new colab.Colab(master, colab_settings));
  // });
