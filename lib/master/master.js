...
Master.prototype.find_server = function (namespace, key) {
  ...
  if (slave) {
    log.debug("%s %s is on %s", namespace, key, slave.ip, slave.colab_port);
    return {
      ip: slave.ip,
...
Master.prototype.get_source = function (workspace) {
  ...
  _.shuffle(_.keys(workspace.slaves)).every(function (colab_id) {
    ...
    if (colab.errors > 0) {
      log.debug("Skipping source %s for %s: Too many errors (%s)", colab.ip, workspace.id, colab.errors);
      return true;
    }
    if (colab.exclude) {
      return true;
