...
var Master = function () {
  ...
  actions.slave.onUPDATE_COUNT(function (id, workspace) {
    var slave = self.slaves[id];

    log.log("Updating count for %s %s", slave.toString(), workspace.id);
    self.update_count(slave, workspace);
  });
