...
Master.prototype.get_action = function (id, w, last_rep, stats) {
  ...
  } else if (high_repcount === settings.repcount + 1) {
    stats.high++;
  ...
  priority = priority || Math.abs(repcount - settings.repcount) + !!_.find(w.slaves, { active: true });
  return priority;
};
...
exports.run = function (api_server, cb) {
  const master = new Master(api_server);
  master.start();
