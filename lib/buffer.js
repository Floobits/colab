...
const CRPatchRegex = new RegExp("%0D", "g");
...
BaseBuffer.prototype.patch = function (agent, req_id, patches, md5_before, md5_after) {
  ...
  if (self.encoding === "utf8") {
    ...
    patches = patches.replace(CRPatchRegex, "");
  }
  ...
  if (self.handle_indentation) {
    patches = self.handle_indentation(agent, patches);
  }
...
TextBuffer.prototype.handle_indentation = function (agent, patches) {
  const self = this;
  const indentation_msg = "Possible indentation mismatch! File is indented with %s, but %s sent %s.";
  if (patches.indexOf("%09") !== -1 && self.detect_indentation() !== "\t") {
    ...
    utils.rate_limit(agent.toString(), settings.indent_error_limit, function () {
      agent.error(null, util.format(indentation_msg, "spaces", "you", "tabs") + " Check your indentation rules!", false);
      const msg = new MSG(agent, util.format(indentation_msg, "spaces", agent.username, "tabs"));
      self.room.broadcast("msg", agent, null, msg.to_json());
    });
    ...
    utils.rate_limit(agent.toString(), settings.indent_error_limit, function () {
      agent.error(null, util.format(indentation_msg, "tabs", "you", "spaces") + " Check your indentation rules!", false);
      const msg = new MSG(agent, util.format(indentation_msg, "tabs", agent.username, "spaces"));
      self.room.broadcast("msg", agent, null, msg.to_json());
    });
    // TODO: probably don't want to always do this. also, fix md5_after
