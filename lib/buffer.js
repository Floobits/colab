...
BaseBuffer.prototype.save = function (create, cb) {
  ...
  async.db = function (cb) {
    self.room.db.put(self.id, {
      id: self.id,
      path: self.path,
      deleted: false,
      md5: self._md5,
  ...
  async.buf_content = ["db", function (cb) {
    self.room.db.put(util.format("buf_content_%s", self.id), self._state, { valueEncoding: self.get_db_encoding() }, cb);
  }];
