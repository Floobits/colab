...
var CRRegex, CRPatchRegex, CommentRegex, EmptyLineRegex, SUPPORTED_ENCODINGS, BaseBuffer, BinaryBuffer, TextBuffer, make_buffer;

CRRegex = new RegExp("\r\n", "g");
CRPatchRegex = new RegExp("%0D%0A", "g");

CommentRegex = new RegExp("\\/\\*[\\S\\s]*?\\*\\/", "g");
EmptyLineRegex = new RegExp("^\\s*$", "g");
...
BaseBuffer.prototype.detect_indentation = function () {
  ...
  buf = self._state.toString("utf8");

  // Kill multi-line comments
  stripped_buf = buf.replace(CommentRegex, "\n");
  // Kill empty lines
  ...
  whitespace = _.map(stripped_buf.split("\n").slice(0, 100), function (line) {
    var counts = {
        tabs: 0,
        spaces: 0
      },
  ...
  log.debug("totals: tabs %s spaces %s", totals.tabs, totals.spaces);
...
BaseBuffer.prototype.dirtify = function () {
  ...
  self.save_timeout = setTimeout(function (err) {
    self.save_timeout = null;
    if (err) {
...
BaseBuffer.prototype.set = function (agent, req_id, state, md5, encoding, broadcast, cb) {
  ...
  if (self.encoding !== encoding) {
    return self.room.delete_buf(agent, req_id, self.id, true, function (err) {
      ...
      self.room.create_buf(agent, req_id, self.path, state, encoding, function (err, result) {
...
BaseBuffer.prototype.patch = function (agent, req_id, patches, md5_before, md5_after) {
  ...
  if (patches.indexOf("%09") !== -1) {
    // Contains a tab!
    indent = self.detect_indentation();
    if (indent !== "\t") {
      log.warn("%s probably has incorrect indentation rules. Patch contains a tab.", agent.toString());
      agent.error(null, "Indentation mismatch! File is indented with spaces, but you sent a tab. Check your indentation rules!", false);
      // TODO: probably don't want to always do this. also, fix md5_after
      patches = patches.replace("%09", encodeURIComponent(indent));
    }
  } else if (patches.indexOf("%20%20%20%20") !== -1) {
    // Contains at least four consecutive spaces. Probably indentation.
    indent = self.detect_indentation();
    if (indent === "\t") {
      log.warn("%s probably has incorrect indentation rules. Patch contains spaces.", agent.toString());
      agent.error(null, "Indentation mismatch! File is indented with tabs, but you sent a spaces. Check your indentation rules!", false);
      // TODO: probably don't want to always do this. also, fix md5_after
      // patches = patches.replace("%20%20%20%20", encodeURIComponent(indent));
    }
    // Horrible flag used later to ignore md5_after
    indent = null;
  }
  ...
  if (self_md5_after !== md5_after) {
    ...
    if (self_md5_before === md5_before && !indent) {
