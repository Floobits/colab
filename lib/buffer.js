...
var CRRegex, CRPatchRegex, SUPPORTED_ENCODINGS, BaseBuffer, BinaryBuffer, TextBuffer, from_db, make;
...
BaseBuffer.prototype.toString = function () {
  ...
  return util.format("Buffer %s %s/%s md5 %s length %s", self.id, self.room, self.path, self._md5, length);
};
...
make = function (room, id, path, text, md5, create, encoding) {
  var state = text;
