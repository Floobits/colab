...
BaseBuffer.prototype.send_get_buf = function (agent, req_id, delay) {
  ...
  self.get_buf_timeouts[agent.id] = {req_id: req_id, timeout_id: setTimeout(function () {
    if (self._state) {
      ...
      log.log("Sent get_buf to %s for %s", agent.toString(), self.toString());
    } else {
...
BaseBuffer.prototype.patch = function (agent, req_id, patches, md5_before, md5_after) {
  ...
  if (patches.indexOf("%09") !== -1 && self.detect_indentation() !== "\t") {
    ...
    utils.rate_limit(agent.toString(), settings.indent_error_limit, function () {
      agent.error(null, util.format(indentation_msg, "you", "spaces", "tabs") + " Check your indentation rules!", false);
      const msg = new MSG(agent, util.format(indentation_msg, agent.username, "spaces", "tabs"));
      self.room.broadcast("msg", agent, null, msg.to_json());
    });
    ...
    utils.rate_limit(agent.toString(), settings.indent_error_limit, function () {
      agent.error(null, util.format(indentation_msg, "you", "tabs", "spaces") + " Check your indentation rules!", false);
      const msg = new MSG(agent, util.format(indentation_msg, agent.username, "tabs", "spaces"));
      self.room.broadcast("msg", agent, null, msg.to_json());
    });
    // TODO: probably don't want to always do this. also, fix md5_after
