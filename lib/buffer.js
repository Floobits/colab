...
BaseBuffer.prototype.load = function (cb) {
  ...
  self.load_local(function (err, result) {
    if (err) {
...
BaseBuffer.prototype.load_db = function (cb) {
  var self = this,
    buf_key = util.format("buf_content_%s", self.id),
    db_encoding = self.encoding === "utf8" ? "utf8" : "binary";

  self.db.get(buf_key, { valueEncoding: db_encoding }, function (err, result) {
    if (err) {
      return cb(err, result);
    }
