...
BaseBuffer.prototype.set = function (req_id, agent, state, md5, encoding, cb) {
  var self = this,
    patches,
  ...
  self.room.broadcast("patch", null, agent, {
    user_id: agent.id,
...
BaseBuffer.prototype.send_get_buf = function (agent, delay) {
  ...
  self.get_buf_timeouts[agent.id] = setTimeout(function () {
    if (agent) {
      agent.write("get_buf", null, self.to_json(agent));
