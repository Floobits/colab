...
ColabBuffer.prototype.send_get_buf = function (agent, delay) {
  ...
  self.get_buf_timeouts[agent.id] = setTimeout(function () {
    if (agent && !agent.disconnected) {
      buf_json = self.to_json();
      buf_json.buf = buf_json.buf.toString();
      agent.write("get_buf", buf_json);
    }
  }, delay || 1500);
};

ColabBuffer.prototype.apply_patches = function (patches, buf) {
  var self = this,
    p,
    native_result,
    js_result;
...
ColabBuffer.prototype.patch = function (agent, patches, md5_before, md5_after) {
  var self = this,
    agent_state,
    agent_result,
    clean_patch = true,
