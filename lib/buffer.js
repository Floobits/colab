  ...
  if (settings.checksum_patches && md5_before !== self_md5_before) {
    ...
    for (i = 0; i < self.prev_bufs.length; i++) {
      ...
      if (prev_buf.md5 === md5_before) {
        ...
        log.debug("applying patch", DMP.patch_toText(patches), "to agent text");
      ...
      if (agent_text) {
        log.debug("agent text is", agent_text);
        next_buf = self.prev_bufs[i + 1];
        if (next_buf && utils.md5(agent_text) === next_buf.md5) {
          log.debug("agent text matches current state. our work here is done.");
          return;
        }
