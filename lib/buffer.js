...
BaseBuffer.prototype.load_s3 = function (cb) {
  ...
  req.on("response", function (res) {
    ...
    res.on("end", function () {
      self._state = new Buffer(data, "binary");
      self._md5 = utils.md5(self._state);
      self.loaded = true;
      cb(null, self);
    });
  ...
  req.on("error", function (err, result) {
    log.error("S3 request error:", err, result);
    cb(util.format("S3 request error:", err), result);
  });
  req.end();
};
  ...
  auto = {
    put: function (cb) {
      ...
      req.on("error", function (err) {
        log.error("Error saving buffer %s to s3: %s", self.guid, err);
      });
      req.end(self._state);
    }
  };
