...
BaseBuffer.prototype.detect_indentation = function () {
  ...
  // Probably need to tweak this
  if (totals.tabs > totals.spaces) {
    log.debug("TABS");
    return "\t";
  }

  for (i = 2; i < 5; i++) {
    good = _.filter(whitespace, function (count) {
      log.debug(count.spaces / i);
      return (count.spaces / i) % 1 === 0;
    }).length;
...
BaseBuffer.prototype.load = function (cb) {
  ...
  self.room.db.get(self.content_key, { valueEncoding: "binary" }, function (err, result) {
    ...
    self.loaded = true;
    self.emit("load");
    log.debug("Indentation is '%s'", self.detect_indentation());
    return cb(null, self);
  });
};
