...
BaseBuffer.prototype.load = function (cb) {
  var self = this,
    buf_key = util.format("buf_content_%s", self.id),
    buf_md5;

  self.room.db.get(buf_key, { valueEncoding: self.get_db_encoding() }, function (err, result) {
    if (err) {
      self.emit("load_error");
      return cb(err, result);
    }
    self._state = result;
    if (_.isUndefined(self._state)) {
      log.warn("No data in buffer %s! Setting to empty.", self.toString());
      self._state = new Buffer("", self.encoding);
    }
...
// TODO: snapshot to S3 occasionaly
...
BaseBuffer.prototype.save = function (create, cb) {
  ...
  if (_.isUndefined(encoding_id)) {
    log.error("Buf %s encoding %s is not valid!", self.toString(), self.encoding);
  }
  ...
  self.room.db.batch()
    ...
    .put(util.format("buf_content_%s", self.id), self._state, { valueEncoding: self.get_db_encoding() })
    .write(cb);
};

BaseBuffer.prototype.set = function (agent, buf, md5, encoding, cb) {
  var self = this,
    patches,
  ...
  if (self.encoding !== encoding) {
    return self.room.delete_buf(self.id, agent, function (err) {
      if (err) {
        return cb(err);
      }
      self.room.create_buf(self.path, buf, encoding, agent, function (err, result) {
  ...
  patches = DMP.patch_make(self._state, buf);
  self._state = buf;
