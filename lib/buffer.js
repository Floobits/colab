...
ColabBuffer.prototype.local_path = function() {
  var self = this;
  return path.normalize(path.join(settings.buf_storage.local.dir, self.guid));
};
...
ColabBuffer.prototype.load_file = function (cb) {
  var self = this;
  fs.readFile(self.local_path(), {encoding: "utf-8"}, function (err, data) {
    if (err) {
      return cb(err, data);
    }
    self._state = data;
...
ColabBuffer.prototype.save = function (create, cb) {
  ...
  if (settings.buf_storage.s3) {
    auto.s3 = ["db", function (cb) {
      self.save_s3(create, cb);
    }];
  }
  if (settings.buf_storage.local) {
    auto.local = ["db", function (cb) {
...
ColabBuffer.prototype.save_s3 = function (create, cb) {
  var self = this,
      auto;
  ...
  auto = {
    put: function (cb) {
      ...
      req.on("error", function (err) {
        ...
        // TODO: save to local disk or something
      });
      req.end(self._state);
    }
  };
...
ColabBuffer.prototype.patch = function (agent, patch_text, md5_before, md5_after) {
  ...
  if (settings.checksum_patches && md5_before !== self_md5_before) {
    ...
    if (prev_result && utils.patched_cleanly(prev_result)) {
      ...
      log.error("We should patch the old version of the buffer, but that code isn't written yet. Soldiering on...");
    } else {
