...
BaseBuffer.prototype.load = function (cb) {
  ...
  self.room.db.get(buf_key, { valueEncoding: "binary" }, function (err, result) {
    if (!err) {
      self._state = result;
...
BaseBuffer.prototype.save = function (create, cb) {
  ...
  batch.put(util.format("buf_%s", self.id), {
    id: self.id,
    path: self.path,
    deleted: false,
    md5: self._md5,
  ...
  if (self._state.length > 0) {
    batch.put(util.format("buf_content_%s", self.id), self._state, { valueEncoding: "binary" });
  } else {
    actions.push(function (cb) {
      self.room.db.del(util.format("buf_content_%s", self.id), function (err) {
