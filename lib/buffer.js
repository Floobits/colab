...
TextBuffer.prototype.handle_newlines = function (agent, patches, md5_before, md5_after) {
  const has_cr = patches.includes(CRPatch);
  if (!has_cr) {
    return [patches, md5_before, md5_after];
  }
  ...
  // TODO: parse patch, fix offsets, kill CRs
  patches = patches.replace(CRPatchRegex, "");
  return [patches, null, null];
};
