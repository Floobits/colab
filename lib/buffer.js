...
BaseBuffer.prototype.set = function (agent, path, buf, md5, encoding) {
  var self = this,
    patches,
  ...
  if (self.encoding !== encoding) {
    return self.room.delete_buf(self.id, agent, function (err) {
      ...
      self.room.create_buf(path, buf, encoding, function () {
  ...
  if (!SUPPORTED_ENCODINGS[encoding]) {
    return agent.error("Unsupported encoding: ", encoding);
  }
...
TextBuffer = function (room, id, path, text, md5, create) {
  var self = this;
...
BinaryBuffer = function (room, id, path, text, md5, create) {
  var self = this;
