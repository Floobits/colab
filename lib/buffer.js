...
var CRRegex, CRPatchRegex, SUPPORTED_ENCODINGS, BaseBuffer, BinaryBuffer, TextBuffer, make_buffer;
...
BaseBuffer.prototype.set = function (req_id, agent, state, md5, encoding, broadcast, cb) {
  var self = this,
    data,
    patches,
  ...
  if (self.encoding !== encoding) {
    return self.room.delete_buf(self.id, agent, true, function (err) {
      ...
      self.room.create_buf(self.path, state, encoding, agent, req_id, function (err, result) {
  ...
  if (broadcast) {
    ...
    self.room.broadcast("patch", req_id, agent, data);
  }
...
BaseBuffer.prototype.send_get_buf = function (req_id, agent, delay) {
  var self = this,
    gbt;
...
BaseBuffer.prototype.patch = function (req_id, agent, patches, md5_before, md5_after) {
  var self = this,
    agent_state,
    agent_result,
    i,
    ...
    for (i = 0; i < self.prev_bufs.length; i++) {
      ...
      if (prev_buf.md5 === md5_before) {
        ...
        if (utils.patched_cleanly(result) === false) {
          ...
          self.send_get_buf(req_id, agent, 0);
          return;
        }
  ...
  if (self_md5_after !== md5_after) {
    ...
    self.send_get_buf(req_id, agent, 1000);
  }
  ...
  self.cancel_get_buf(agent);
  self.room.broadcast("patch", req_id, agent, {
    user_id: agent.id,
    username: agent.username,
    md5_before: self_md5_before,
...
BaseBuffer.prototype.highlight = function (agent, req) {
  ...
  self.room.broadcast("highlight", null, agent, highlight);
};
