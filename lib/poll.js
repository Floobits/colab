...
var ColabPoller = function (controller, interval) {
  var self = this;
...
ColabPoller.prototype.update_workspace_counts = function (colab_server, body, cb) {
  var self = this;

  // Filter out current server from workspace info. Probably a better way to do this.
  _.each(self.controller.workspaces, function (w) {
  ...
  _.each(body.workspaces, function (workspace) {
    ...
    if (workspace.owner && workspace.name) {
      ...
      self.controller.server_mapping.workspace[key] = colab_server;
    }
    if (colab_server.exclude) {
      return;
    }
    w = self.controller.workspaces[workspace.id];
    if (!w) {
      w = {
        id: workspace.id,
        servers: {}
      };
      self.controller.workspaces[workspace.id] = w;
    }
...
ColabPoller.prototype.replicate = function (cb) {
  var self = this,
    actions = [],
    stats = {
      high: 0,
  ...
  log.debug(self.controller.workspaces);
  _.each(self.controller.workspaces, function (w, id) {
    ...
    if (repcount < settings.repcount) {
      ...
      actions.push(function () {
        self.controller.copy_workspace(_.shuffle(w.servers)[0], null, id);
      });
      ...
      actions.push(function () {
        self.controller.delete_workspace_by_id(_.shuffle(w.servers)[0], id);
      });
    } else {
      stats.correct++;
  ...
  log.log("Ran %s actions.", actions.length);
  return cb();
};
...
ColabPoller.prototype.poll = function (cb) {
  ...
  async.each(self.controller.colab_servers, function (colab_server, cb) {
    ...
    request.get(url, options, function (err, response, body) {
      ...
      if (!!colab_server.id && colab_server.id !== body.server_id) {
        log.error("Server ID changed from %s to %s!", colab_server.id, body.server_id);
      }
      colab_server.id = body.server_id;

      return self.update_workspace_counts(colab_server, body, cb);
    });
  }, function (err) {
    if (err) {
