...
ColabServer.prototype.load_workspace = function (agent, api_workspace, cb) {
  var self = this,
    RoomClass = Room,
    workspace;


  workspace = self.workspaces[api_workspace.id];
  if (workspace) {
    if (workspace.state === ROOM_STATES.LOADING) {
      return workspace.once("load", cb);
    }
    if (workspace.state === ROOM_STATES.LOADED) {
      // In case it was updated
      workspace.max_size = api_workspace.max_size;
  ...
  if (api_workspace.pro) {
    RoomClass = ProRoom;
  }
  workspace = new RoomClass(api_workspace.id,
    api_workspace.name,
    api_workspace.owner, {
  ...
  self.workspaces[api_workspace.id] = workspace;
...
ColabServer.prototype.on_add_agent = function (owner, name, agent, user, cb) {
  ...
  function finish(err, result) {
    ...
    result.add_agent(agent, user, cb);
  }
  ...
  api_client.workspace_get(owner, name, function (err, result) {
    ...
    self.load_workspace(agent, result, finish);
  });
};

ColabServer.prototype.on_add_colab = function (workspace_id, agent, colab_id, cb) {
  var self = this;
  ...
  api_client.workspace_get_by_id(workspace_id, function (err, result) {
    if (err) {
      return cb(err);
    }
    ...
    if (result === undefined) {
      log.error("Workspace id not found for id", workspace_id);
      return cb("Workspace not found");
    }
    self.load_workspace(agent, result, finish);
  });
