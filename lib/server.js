...
ColabControlServer.prototype.move_workspace = function (workspace, source, dest, cb) {
  ...
  log.debug("Moving workspace %s from %s to %s", workspace, source.toString(), dest.toString());
  return cb();
  auto.copy_workspace = function (cb) {
...
ColabControlServer.prototype.rebalance = function () {
  ...
  async.forEach(high, function (colab, cb) {
    ...
    async.forEach(_.sample(colab.workspaces, 10), function (w, cb) {
      var candidates = _.values(self.colabs),
        dest;
      w = self.workspaces[w.id];
      if (_.size(low) > 0) {
        candidates = _.map(low, function (c) {
          return self.colabs[c];
        });
      }
      ...
      self.move_workspace(w, colab, dest, cb);
    }, function (err, result) {
      if (err) {
