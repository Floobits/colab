...
exports.run = function () {
  ...
  auto.db_check = ["get_server_id", function (cb, response) {
            ...
            db.get(util.format("buf_content_%s", buf_id), { valueEncoding: "binary" }, function (err, result) {
              ...
              if (expected_md5 !== md5) {
                if (md5 === "d41d8cd98f00b204e9800998ecf8427e") {
                  log.error("Buf %s/%s is empty!", workspace_id, buf_id);
                } else {
                  log.error("Buf %s/%s md5 mismatch! Expected %s. Got %s", workspace_id, buf_id, expected_md5, md5);
                  if (utils.md5(result.toString()) === expected_md5) {
                    log.warn("Encoding issue!");
                  }
                }
              }
            });
          });
          brs.on("close", function () {
            ldb.finish_db(db, workspace_id);
          });
        });
      });
  ...
  auto.listen = ["cache_connect", "get_server_id", "db_check", "fs_check", function (cb, response) {
