...
ColabControlServer.prototype.find_server = function (namespace, key) {
  ...
  if (namespace === "workspace") {
    ...
    // TODO: split the important logic out of get_copy_candidates
    candidates = self.get_copy_candidates(workspace);
    colab_server = candidates.source;
    if (!colab_server) {
      return null;
    }
    return {
...
ColabControlServer.prototype.handle_get_workspace = function (req, res) {
  ...
  data = self.find_server("workspace", req.params.workspace_id);
  if (!data) {
    return res.json(404);
  }
...
ColabControlServer.prototype.handle_get_workspace_by_name = function (req, res) {
  var self = this,
  ...
  db.get_workspace(owner, workspace, function (err, result) {
    ...
    data = self.find_server("workspace", result.id);
    if (!data) {
      return res.json(404);
    }
...
ColabControlServer.prototype.handle_evict_workspace = function (req, res) {
  ...
  return self.evict_workspace(server, req.params.workspace_id, function (err, result) {
    if (err) {
...
ColabControlServer.prototype.handle_delete_workspace_by_name = function (req, res) {
  var self = this,
    owner = req.params.owner,
    workspace = req.params.workspace;
  ...
  db.get_workspace(owner, workspace, function (err, result) {
    ...
    workspace = self.workspaces[result.id];

    if (!workspace || !workspace.colabs || _.size(workspace.colabs) === 0) {
      return res.json(404, {error: util.format("Server not found for workspace %s", result.id)});
    }

    async.forEach(workspace.colabs, function (colab, cb) {
      self.delete_workspace(workspace, colab, cb);
    }, function (err, result) {
      if (err) {
        return res.send(err, result);
      }
...
ColabControlServer.prototype.handle_pin_workspace = function (req, res) {
  ...
  return self.evict_workspace(old_server, req.params.workspace_id, function (err, result) {
    if (err) {
      return res.send(err, result);
    }
    return res.json(200, server);
...
ColabControlServer.prototype.handle_get_stats = function (req, res) {
  ...
  stats.actions = _.map(_.filter(self.workspaces, function (w) {
    return !!w.action && !!w.action.to;
  }), function (w) {
    return w.action;
  });

  stats.action_history = self.action_history;
...
ColabControlServer.prototype.rebalance = function () {
  ...
  async.forEach(high, function (colab, cb) {
    ...
    colab = self.colab_servers[colab];
    ...
    async.forEach(_.sample(colab_workspaces, 50), function (w, cb) {
      ...
      if (_.size(low) > 0) {
        candidates = _.map(low, function (c) {
          return self.colab_servers[c];
        });
      } else {
        candidates = _.values(self.colab_servers);
      }
...
ColabControlServer.prototype.replicate = function () {
  ...
  _.each(self.workspaces, function (w, id) {
    ...
    if (self.backup && backup_repcount === 0) {
      ...
      // No backup? UNACCEPTABLE
      action_workspaces[10] = action_workspaces[10] || [];
      action_workspaces[10].push(w);
    }
    ...
    if (repcount === 0) {
      ...
      stats.correct++;
      if (w.action && _.contains(["copy", "delete"], w.action.name)) {
  ...
  prioritized_actions = prioritized_actions.slice(0, 100);
