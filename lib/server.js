...
ColabControlServer.prototype.evict_workspace = function (server, owner, workspace, cb) {
  var options = {
    ...
    url = util.format("%s://%s:%s/r/%s/%s/evict", (server.ssl ? "https" : "http"), server.ip, server.api_port, owner, workspace);
...
ColabControlServer.prototype.delete_workspace_by_name = function (server, owner, workspace, cb) {
  var options = {
    ...
    url = util.format("%s://%s:%s/r/%s/%s", (server.ssl ? "https" : "http"), server.ip, server.api_port, owner, workspace);
  ...
  request.del(url, options, function (err, response, body) {
    ...
    log.debug("Response from DELETE %s: %s", url, JSON.stringify(body));
...
ColabControlServer.prototype.get_backup_candidates = function (workspace) {
  ...
  candidates = _.filter(self.colab_servers, function (colab) {
    return colab.backup;
  });
  return
};
