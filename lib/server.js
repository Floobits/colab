...
ColabControlServer.prototype.delete_workspace = function (server, owner, workspace, cb) {
  var self = this,
    url = util.format("%s:%s/r/%s/%s", server.ip, server.command_port, owner, workspace);

  request.del({url: url, auth: {user: AUTH_USER, password: AUTH_PASS}}, function (err, response, body) {
    if (err) {
      return cb(500, err);
    }

    log.debug("Response from %s: %s", url, body);
