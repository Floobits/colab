...
ColabServer.prototype.connect_to_master = function (cb) {
  ...
  try {
    cleartext_stream = tls.connect(options, function () {
      agent_conn = new FloobitsProtocol(number, cleartext_stream, self);
      agent_conn.handler = new SlaveHandler(agent_conn);
      return cb();
    });
  } catch (e) {
    return cb(e);
...
ColabServer.prototype.on_conn = function (conn) {
  ...
  agent_conn = new FloobitsProtocol(number, conn, self);
