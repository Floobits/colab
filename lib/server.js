...
ColabControlServer.prototype.find_server = function (namespace, key) {
  ...
  colab_server = _.find(_.shuffle(self.colab_servers), function (server) {
  ...
  if (colab_server) {
    log.debug(util.format("Picked %s for %s:%s", colab_server.ip, namespace, key));
  } else {
    log.warn("All servers are busy. Picking one at random.");
    colab_server = _.sample(self.colab_servers);
    log.debug(util.format("Randomly picked %s for %s:%s", colab_server.ip, namespace, key));
  }
