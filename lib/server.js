...
var ColabControlServer = function () {
  ...
  self.usernames_to_servers = {};
  self.tokens_to_servers = {};
  ...
  self.app.get('/t/:token', self.handle_token_request.bind(self));
  self.app.get('/u/:username', self.handle_user_request.bind(self));
};
  ...
  async.each(settings.colab_servers, function (colab_server, cb) {
    ...
    req = request.get(url, options, function (err, response, body) {
      ...
      _.each(body.workspaces, function (workspace) {
        ...
        self.workspaces_to_servers[key] = {
          ip: colab_server.ip,
          colab_port: colab_server.colab_port,
...
ColabControlServer.prototype.handle_token_request = function (req, res) {
  var self = this,
    colab_server,
    data,
    server,
  ...
  server = self.tokens_to_servers[token];

  if (_.isUndefined(server)) {
    colab_server = settings.colab_servers[0];
    // TODO: make sure this doesn't get overwritten by a poll happening right after this response
    self.tokens_to_servers[token] = {
      ip: colab_server.ip,
      colab_port: colab_server.colab_port,
      ssl: colab_server.ssl
    };
...
ColabControlServer.prototype.handle_user_request = function (req, res) {
  var self = this,
    colab_server,
    data,
    server,
  ...
  server = self.usernames_to_servers[username];

  if (_.isUndefined(server)) {
    colab_server = settings.colab_servers[0];
    // TODO: make sure this doesn't get overwritten by a poll happening right after this response
    self.usernames_to_servers[username] = {
      ip: colab_server.ip,
      colab_port: colab_server.colab_port,
      ssl: colab_server.ssl
    };
