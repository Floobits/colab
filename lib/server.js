...
ColabServer.prototype.load_rooms = function (cb) {
  var self = this;
  var query = db.client.query("SELECT room_room.id as rid, room_buffer.id as rbid, room_buffer.deleted as rbdeleted, * FROM room_room LEFT OUTER JOIN room_buffer ON room_buffer.room_id = room_room.id;");
  query.on("row", function (row) {
    ...
    if (r === undefined) {
      ...
      log.debug("room", r.id, "already exists");
    }
    if (row.rbid && row.rbdeleted === false) {
