  ...
  async.each(settings.colab_servers, function (colab_server, cb) {
    ...
    req = request.get(url, options, function (err, response, body) {
      ...
      _.each(body.workspaces, function (workspace) {
        ...
        self.colab_servers.workspace[key] = colab_server;
      });
...
ColabControlServer.prototype.find_server = function (namespace, key) {
  var self = this,
    colab_server = self.colab_servers[namespace][key];

  if (_.isUndefined(colab_server)) {
    ...
    self.colab_servers[namespace][key] = colab_server;
  }
