...
exports.run = function () {
  ...
  auto.leveldb_open = function (cb) {
    return levelup(settings.server_db_dir, cb);
  };
    ...
    response.leveldb_open.get("server_id", function (err, result) {
      if (err) {
        if (err.type === "NotFoundError") {
          server.id = util.format("%s_%s", os.hostname(), ("00000" + (Math.random() * 100000)).slice(-6));
          return response.leveldb_open.put("server_id", server.id, cb);
        }
        return cb(err);
      }
      server.id = result;
  ...
  auto.listen = ["cache_connect", "leveldb_open", function (cb, response) {
    server.db = response.leveldb_open;
    server.listen();
    cb();
  }];
