...
ColabControlServer.prototype.poll = function (cb) {
  ...
  async.each(self.colab_servers, function (colab_server, cb) {
    ...
    _.each(self.workspaces, function (w, id) {
      repcount[id] = _.size(w.servers);
    });
    log.debug(self.workspaces);
    log.debug(repcount);
    _.each(repcount, function (count, id) {
      if (count < settings.repcount) {
        log.debug("Workspace %s has %s replicas (not enough).", id, count);
      } else if (count > settings.repcount) {
        log.debug("Workspace %s has %s replicas (too many).", id, count);
      } else {
        log.debug("Workspace %s has the right number of replicas (%s)", id, count);
      }
    });
    cb(err, result);
  });
