...
ColabControlServer.prototype.duplicate_workspace = function (workspace, cb) {
  ...
  _.each(workspace.colabs, function (colab, colab_id) {
    ...
    if (dest && colab.version < dest.version) {
      dest = colab;
      dest_id = colab_id;
    }
  });
  ...
  if (!dest) {
    _.each(self.colab_servers, function (colab, colab_id) {
      if (colab.poller.errors > 0 || _.contains(_.keys(workspace.colabs), colab_id)) {
        return;
      }
      if (!dest) {
        dest = colab;
  ...
  _.each(self.workspaces, function (w, id) {
    ...
    _.each(w.colabs, function (colab, colab_id) {
      ...
      if (version < colab.version) {
        log.debug("different version: %s %s", version, colab.version);
        version = colab.version;
        return;
      }
      if (!scolab || scolab.poller.errors > settings.colab_error_threshold) {
        log.debug("no scolab or too many poller errors");
        return;
      }
      repcount++;
    });
