...
const ColabServer = function () {
  ...
  self.server_db_dir = path.join(settings.base_dir, "server_db");
  fs.mkdirsSync(self.server_db_dir);
  ...
  if (settings.ssh_pubkey && settings.ssh_privkey) {
    ...
    if (_.isString(settings.ssh_pubkey) && _.isString(settings.ssh_privkey)) {
      let ssh_key_path = path.join(settings.base_dir, "id_rsa_floobot");
      ssh_config += util.format("IdentityFile %s\n", ssh_key_path);
      fs.writeFileSync(ssh_key_path, settings.ssh_privkey);
      fs.chmodSync(ssh_key_path, "600");
      fs.writeFileSync(ssh_key_path + ".pub", settings.ssh_pubkey);
      fs.chmodSync(ssh_key_path + ".pub", "600");
      log.log("SSH keys written to %s", ssh_key_path);
    } else if (_.isArray(settings.ssh_pubkey) && _.isArray(settings.ssh_privkey) && settings.ssh_pubkey.length === settings.ssh_privkey.length) {
      let i;
      for (i = 0; i < settings.ssh_pubkey.length; i++) {
        let ssh_pubkey = settings.ssh_pubkey[i];
        let ssh_privkey = settings.ssh_privkey[i];
        let host = ssh_pubkey.host;
        if (host !== ssh_privkey.host) {
          throw new Error(util.format("Bad ssh config! Pubkey host %s doesn't match privkey host %s!", host, ssh_privkey.host));
        }
        let ssh_key_path = path.join(settings.base_dir, "id_rsa_floobot_" + i);

        fs.writeFileSync(ssh_key_path, ssh_privkey.key);
        fs.chmodSync(ssh_key_path, "600");
        fs.writeFileSync(ssh_key_path + ".pub", ssh_pubkey.key);
        fs.chmodSync(ssh_key_path + ".pub", "600");
        log.log("SSH keys written to %s", ssh_key_path);

        ssh_config += util.format("Host %s\n", host);
        if (host !== "*") {
          ssh_config += util.format("HostName %s\n", host);
        }
        ssh_config += util.format("IdentityFile %s\n", ssh_key_path);
      }
    } else {
      throw new Error("Bad ssh config!");
    }

    let ssh_config_path = path.join(settings.base_dir, "floobot_ssh.conf");
    fs.writeFileSync(ssh_config_path, ssh_config);
    fs.chmodSync(ssh_config_path, "644");

    settings.ssh_wrapper_path = path.join(settings.base_dir, "id_rsa_floobot.sh");
    fs.writeFileSync(settings.ssh_wrapper_path, util.format("#!/bin/sh\nssh -F %s \"$@\"\n", ssh_config_path));
...
ColabServer.prototype.open_db = function (cb) {
  log.log("Opening %s...", this.server_db_dir);
  level(this.server_db_dir, {
    createIfMissing: true
  }, (err, result) => {
    if (err) {
      return cb(err);
    }
    log.log("Server DB Opened %s.", this.server_db_dir);
    this.db = result;
    return cb(null, result);
  });
};

ColabServer.prototype.get_server_id = function (cb) {
  this.db.get("server_id", (db_open_err, result) => {
    if (!db_open_err) {
      this.id = result;
      log.log("Server ID is %s", this.id);
      return cb(null, result);
    }
    ...
    this.id = server_id;
    log.log("Server ID set to %s", this.id);
    return this.db.put("server_id", server_id, (err) => cb(err, server_id));
  });
};
...
const run = function () {
  ...
  function shut_down(sig) {
    ...
    server.master_reconnect_timeout = true;
  ...
  if (settings.is_master) {
    master = new Master(api_server);
    master.start();
  }

  let auto = {};

  auto.leveldb_open = server.open_db.bind(server);
  auto.get_server_id = ["leveldb_open", server.get_server_id.bind(server)];
  ...
  auto.workspace_stats = ["get_server_id", function (cb) {
    cb = _.once(cb);
    ...
    const rs = server.db.createReadStream({
      gte: "version_",
      lte: "version_999999999999"
    });
    ...
    rs.on("error", function (err) {
      log.error("Error reading %s: %s", server.server_db_dir, err);
      return cb(err, stats);
    });
  }];
  ...
  auto.db_check = ["workspace_stats", function (db_check_cb, response) {
    ...
    async.eachSeries(stats.has_version, function (workspace_id, cb) {
      ...
      fs.exists(p, function (exists) {
        if (!exists) {
          log.error("Workspace %s is in DB but missing dir on filesystem (%s)", workspace_id, p);
        ...
        ldb.get_db(null, workspace_id, null, function (err, db) {
          if (err) {
            log.error("Workspace %s is in DB but leveldb is corrupt or doesn't exist! Error: %s", workspace_id, err);
  ...
  auto.fs_check = ["db_check", function (cb, response) {
    fs.readdir(settings.bufs_dir, function (err, workspaces) {
      ...
      _.each(workspaces, function (workspace) {
        ...
        // TODO: delete or publish that we have version 0
        if (!_.includes(stats.has_version, workspace_id)) {
          log.error("Workspace %s on disk but has no version", workspace);
  ...
  auto.fs_cleanup = ["get_server_id", "fs_check", function (cb, response) {
    ...
    _.each(stats.bad_db, function (bad_db) {
      log.error("Bad DB for workspace %s. Deleting.", bad_db);
    ...
    _.each(stats.missing_dir, function (missing) {
      log.error("Missing dir for workspace %s. Deleting.", missing);
