...
var ColabServer = function () {
  ...
  actions.room.onADD_AGENT(function (owner, name, agent, user, cb) {
    var room;
    ...
    api_client.workspace_get(owner, name, function (err, result) {
      ...
      room = self.workspaces[db_room.id];
      if (room) {
      ...
      self.workspaces[db_room.id] = room;
      room.once("load", finish);
      room.load(agent);
    });
  });
  actions.room.onADD(function (id, room) {
    self.workspaces[id] = room;
  }, this);
  actions.room.onREMOVE(function (id, room) {
  ...
  self.reconnect_to_master_timeout = null;
};
...
ColabServer.prototype.connect_to_master = function (cb) {
  ...
  try {
    cleartext_stream = tls.connect(options, function () {
      ...
      self.slave.once("close", self.reconnect_to_master.bind(self));
      cb();
    });
...
ColabServer.prototype.reconnect_to_master = function () {
  var self = this;

  if (self.reconnect_to_master_timeout) {
    return;
  }
  self.reconnect_delay = Math.min(self.reconnect_delay * 1.5, MAX_RECONNECT_DELAY);
  self.reconnect_to_master_timeout = setTimeout(self.connect_to_master.bind(self), self.reconnect_delay);
};
...
exports.run = function () {
  ...
  function shut_down(sig) {
    ...
    clearTimeout(server.reconnect_to_master_timeout);
