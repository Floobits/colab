...
var ColabServer = function () {
  ...
  actions.broadcast.onSEND_TO_MASTER(function (data, cb) {
    if (!self.slave.handler) {
      return new Error("Not connected to master!");
    }
    self.slave.handler.broadcast(data, cb);
  });

  actions.broadcast.onSEND_TO_WORKSPACES(function (data) {
    var name = data.name;
    _.each(self.workspaces, function (w) {
      w.broadcast(name, null, null, data, true);
    });
  });
};
...
ColabServer.prototype.master_connect = function (cb) {
  ...
  try {
    cleartext_stream = tls.connect(options, function () {
      ...
      self.master_reconnect_delay = INITIAL_RECONNECT_DELAY;
      actions.broadcast.send_to_master({"solicit": "asdf"}, log.error);
      cb();
    });
