...
var metrics_server = require("./metrics_server");
...
var ColabServer = function () {
  ...
  if (settings.metrics_port) {
    ...
    metrics_server.listen(settings.metrics_port, self);
  }
};
...
exports.run = function () {
  ...
  // TODO: integrity check buffer md5s vs buffer contents
  ...
  auto.version_rs = ["get_server_id", function (cb, response) {
    ...
    rs.on("data", function (data) {
      var workspace_id = parseInt(data.key.slice(8), 10),
        p = path.join(settings.bufs_dir, data.key.slice(8));
      ...
      fs.exists(p, function (exists) {
        if (!exists) {
          log.error("%s is in DB but missing on filesystem", p);
          stats.missing.push(workspace_id);
          return;
        }
        ldb.get_db(null, workspace_id, null, function (err, db) {
          if (err) {
            log.error("%s is in DB but leveldb is corrupt or doesn't exist!", workspace_id);
    ...
    rs.on("close", function (err) {
      log.log("Verified %s workspaces.", i);
      stats.total = i;
      return cb(err, stats);
    });
  ...
  auto.listen = ["cache_connect", "get_server_id", "version_rs", function (cb, response) {
    var stats = response.version_rs,
      total_errors = stats.bad_db.length + stats.missing.length;
    _.each(stats.bad_db, function (bad_db) {
      log.error("Bad DB for %s", bad_db);
    });
    _.each(stats.missing, function (missing) {
      log.error("Missing dir for %s", missing);
    });
