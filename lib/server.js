...
var ColabControlServer = function () {
  ...
  _.each(settings.colab_servers, function (colab_settings) {
    self.colabs.push(new colab.Colab(self, colab_settings));
  });
...
ColabControlServer.prototype.copy_workspace = function (server, source, workspace_id, cb) {
  var self = this,
    options = {
      auth: {
        user: AUTH_USER,
    ...
    source_id,
    url = util.format("%s://%s:%s/fetch/%s", (server.ssl ? "https" : "http"), server.ip, server.command_port, workspace_id);

  if (!source) {
    source_id = _.shuffle(_.keys(self.workspaces[workspace_id].servers))[0];
    source = self.servers[source_id];
  }
...
ColabControlServer.prototype.replicate = function (cb) {
  var self = this,
    actions = [],
    stats = {
      high: 0,
  ...
  _.each(self.workspaces, function (w, id) {
    ...
    if (repcount < settings.repcount) {
      ...
      actions.push(function () {
        self.copy_workspace(_.shuffle(w.servers)[0], null, id);
      });
      ...
      actions.push(function () {
        self.delete_workspace_by_id(_.shuffle(w.servers)[0], id);
      });
    } else {
      stats.correct++;
...
exports.run = function () {
  ...
  process.on("SIGHUP", function (sig) {
    ...
    // TODO: refresh server.colab_servers
  });

  log.log("Polling servers...");

  async.each(server.colabs, function (colab, cb) {
    colab.poller.poll(cb);
  });
