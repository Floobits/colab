...
var ColabTerm = function (room, id, owner, name, size) {
  ...
  self.owner.on("on_conn_end", function () {
    self.room.delete_term(self.owner, null, self.id);
  });
};
...
ColabTerm.prototype.update = function (agent, data) {
  ...
  self.room.broadcast("update_term", agent, null, {
    id: self.id,
    user_id: agent.id,
...
ColabTerm.prototype.stdin = function (agent, data) {
  ...
  self.room.broadcast("term_stdin", agent, null, {
    id: self.id,
    user_id: agent.id,
    username: agent.username,
    data: ""
...
ColabTerm.prototype.stdout = function (data) {
  ...
  self.room.broadcast("term_stdout", self.owner, null, {
    id: self.id,
    data: data
  });
};
