...
const ProRoom = function () {
  ...
  this.once("load", this.load_solicitations.bind(this));
};
...
ProRoom.prototype.add_agent = function (agent, user, cb) {
  const solicitations = this.solicitations;
  Room.prototype.add_agent.call(this, agent, user, function (err, res) {
    if (err) {
      return cb(err, res);
    }

    _.each(solicitations, function (s) {
...
ProRoom.prototype.load_solicitations = function (cb) {
  var self = this;
  cb = cb || function () {
    return;
  };
  // TODO: possible race condition if this takes a while. maybe we should put it in room load?
  api_client.solicitations_get_active(function (err, result) {
    if (err) {
      return cb(err, result);
    }
    _.each(result, function (data) {
      ...
      self.broadcast("solicit", null, null, {
        data: s.to_json(),
      });
    });
    return cb(err, result);
...
ProRoom.prototype.solicit = function (agent, req_id, req, cb) {
  ...
  const f = s[util.format("on_%s", req.action)];
  if (!_.isFunction(f)) {
