...
Room.prototype.create_buf = function (path, fid, text) {
  ...
  if (fid === undefined) {
    ...
    db.query("UPDATE room_room SET cur_fid = $1 WHERE id = $2", [self.cur_fid, self.id], function (err, result) {
      log.debug(result);
      if (err) {
        log.error(err);
      }
  ...
  buf = new ColabBuffer(self, path, fid, text);
...
exports.add_agent = function (owner, name, agent) {
  var room_id;
  var room;
  room_id = db.get_room(owner, name);
  room = agent.server.rooms[room_id];
