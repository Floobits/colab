  ...
  ldb.read_events(self.db, self.id, function (read_events_err, rs) {
    ...
    rs.on("data", function (data) {
      ...
      try {
        ...
        log.error("Error parsing event %s: %s", data, e);
        return;
      }
      if (_.includes(PERSISTENT_EVENTS, row.name)) {
        let evt;
        try {
  ...
  ldb.read_buf_info(self.db, self.id, function (buf_info_err, rs) {
    ...
    rs.on("data", function (data) {
      ...
      try {
        ...
        log.error("Error parsing buf info %s: %s", JSON.stringify(data), e);
        return;
      }
...
Room.prototype.broadcast = function (name, source, req_id, data, broadcast) {
  ...
  broadcast = _.isUndefined(broadcast) ? _.includes(BROADCAST_ACTIONS, name) : broadcast;

  if (_.includes(PERSISTENT_EVENTS, name)) {
...
Room.prototype.check_path = function (_path) {
  ...
  _.each(_path.split("/"), function (chunk) {
    if (_.includes(path_chunk_blacklist, chunk)) {
...
Room.prototype.save_bufs = function (save_cb) {
  ...
  log.debug("saving bufs for %s", self.toString());
  if (self.state >= ROOM_STATES.DESTROYING) {
  ...
  async.eachLimit(_.values(self.bufs), 20, function (buf, cb) {
    log.debug("%s Saving buf %s", self.toString(), buf.toString());
    buf.save(false, function (err) {
      log.debug("%s Saved buf %s", self.toString(), buf.toString());
      buf.cancel_timeouts();
      if (err) {
...
Room.prototype.save_events = function (cb) {
  const self = this;
  log.debug("%s Saving events", self.toString());

  async.each(self.events, (evt, evt_cb) => {
    if (evt.name !== "msg") {
      return evt_cb();
    }
    const key = util.format("event_%s", evt.id);
    self.db.put(key, evt.to_db(), {
      valueEncoding: "json",
    }, evt_cb);
    log.debug("Saving event %s", evt.toString());
  }, err => {
    log.debug("%s Saved events. err: %s", self.toString(), err);
    return cb(err);
  });
};
...
Room.prototype.on_highlight = function (agent, req_id, req, cb) {
  ...
  if (req.to) {
    _.each(self.handlers, function (a, k) {
      ...
      if (_.includes(req.to, a.username)) {
...
Room.prototype.on_webrtc = function (agent, req_id, data) {
  ...
  if (!_.includes(agent.perms, "patch") && _.includes(patchActions, data.action)) {
  ...
  if (data.action !== "stop") {
    agent.video_chatting = !_.includes([
      "quit",
      "reject",
    ], data.action);
  }
...
Room.prototype.set_repo = function(agent, req_id, data) {
  // Room.prototype.broadcast = function (name, source, req_id, data, broadcast) {
  if (!_.isObject(data)) {
    agent.error(req_id, "Bad request");
    return;
  }
  // set repo and pull
  const broadcast = () => {
    ...
    this.broadcast("repo", agent, req_id, {
      action: "set",
      data: this.repo.to_json(),
    });
  };
  this.save((err) => {
    if (err) {
...
Room.prototype.pull_repo = function(agent, req_id, req) {
  const self = this;
  function cb(err) {
    if (err) {
      log.error("%s error updating repo for %s: %s", agent.toString(), self.toString(), err);
      return agent && agent.error(req_id, err, true);
    }
    log.log("%s updated repo for %s", agent.toString(), self.toString());
    if (self.save) {
      self.save();
    } else {
    ...
    return agent && agent.ack(req_id);
  }

  if (self.repo && self.repo.is_equal(req)) {
    self.repo.update(agent, req_id, cb);
  } else {
    try {
      self.repo = new Repo(self, req, self.repo && self.repo.private_github_url);
    } catch (e) {
      agent.error(req_id, "Error creating repository:" + e.toString(), true);
      return;
    }
    self.repo.clone(agent, req_id, cb);
  }
};
...
Room.prototype.get_admins = function () {
  ...
  return _.filter(self.handlers, function (user) {
    return _.includes(user.perms, "perms");
  });
};
...
Room.prototype.add_agent = function (agent, user, add_agent_cb) {
  ...
  return async.parallel({
    ...
    if (agent.is_anon === false && _.includes(res.agent_perms, "view_room")) {
  ...
  f.call(s, agent, data, function (err, result) {
    ...
    switch (req.action) {
      ...
      case "cancel":
        ...
        break;
      case "bid":
      case "feedback": // TODO: allow feedback from contractors
      case "complete": // TODO: prune completed
      case "hire":
        break;
      case "keep_alive":
