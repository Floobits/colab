...
var Room = function (id, name, owner, atts) {
  ...
  self.repo_url = atts.repo_url;

  if (self.repo_url) {
    self.repo = new Repo(self, self.repo_url);
    self.repo.update(function (err, result) {
      if (err) {
        // TODO: This is bad! do something!
        log.error("Shiiiiiiiiiit", err, result);
        return;
      }
    });
  }
...
Room.prototype.get_buf_by_path = function (path) {
  var self = this;
  return _.find(self.bufs, function (buf, id) {
      ...
      if (agent && agent.conn && agent.conn.manager) {
        switch (agent.conn.manager.server) {
          ...
          case agent.server.server_ssl:
            log.debug("Agent", agent, "is on a secure connection.");
          break;
          default:
        ...
        if (room.name === "yc_demo") {
          log.debug("SOMEONE JOINED YC DEMO ROOM");
            ...
            form: {
              From: "+15108783772",
              To: "+14153435517",
              Body: "SOMEONE IS LOOKING AT YC DEMO!!!!!1111"
            }
          });
        }
      }
  ...
  db.get_room(owner, name, function (err, result) {
    ...
    return db.client.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      ...
      if (result.rows.length === 0) {
        ...
        return room.create_buf("README.md", settings.readme_text, finish);
      }
