...
Room.prototype.create_buf = function (path, text, encoding, agent, cb) {
  ...
  try {
    buf = make_buffer(self, fid, path, new Buffer(text), undefined, true, encoding);
  } catch (e) {
    log.error(e);
    ...
    return db.client.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      ...
      return async.eachLimit(result.rows, 20,
        function (row, callback) {
          var buf;
          try {
            buf = make_buffer(room, row.fid, row.path, new Buffer(0), row.md5, false, utils.db_encodings_mapping[row.encoding]);
          } catch (e) {
            return callback(e);
          }
