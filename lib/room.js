...
Room.prototype.create_buf = function (path, fid, text, cb) {
  var self = this;
  var buf;
  var err;
  var db_create = false;
  ...
  _.each(self.bufs, function (_buf) {
    if (_buf.path === path) {
      err = "Duplicate path: buffer " + _buf.id + " already has path " + path;
      log.error(err);
    }
  });
  if (err) {
...
Room.prototype.save_bufs = function() {
  var self = this;
...
exports.add_agent = function (owner, name, agent, cb) {
  ...
  finish = function(err) {
    ...
    db.get_perms(agent.user_id, room.id, function (err, result) {
      if (err) {
        log.error(err);
        return cb();
      }
  ...
  db.get_room(owner, name, function (err, result) {
    ...
    db.client.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      if (err) {
        return log.error("error getting buffers for room", db_room.id, err);
      }
      var s3_client = s3.get_client();

      async.each(result.rows, function (row, callback) {
        var buf_guid = util.format("%s-%s", db_room.id, row.fid);
        var req = s3_client.get(buf_guid);
        ...
        req.on("response", function (res) {
          ...
          res.on("end", function () {
            room.create_buf(row.path, row.fid, data, callback);
          });
