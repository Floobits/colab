...
Room.prototype.loadBufs = function (agent, cb) {
  ...
  rs.on("data", function (data) {
    var buf,
      row = data.value;
    try {
      buf = make_buffer(self, row.fid, row.path, new Buffer(0), row.md5, false, db.buf_encodings_mapping[row.encoding]);
    } catch (e) {
    ...
    buf.load(function (err) {
      if (err) {
        self.evict(util.format("Error loading buffer %s: %s", buf.toString(), err));
      }
