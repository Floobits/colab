...
var db_perms_mapping = {
  0: [],
  1: ["get_buf"],
  2: ["patch", "get_buf", "create_buf", "highlight", "msg"]
};
...
Room.prototype.create_buf = function (path, fid, text) {
  ...
  if (fid === undefined) {
    ...
    db.client.query("UPDATE room_room SET cur_fid = $1 WHERE id = $2", [self.cur_fid, self.id], function (err, result) {
      ...
      if (err) {
        log.error(err);
        // With our current exception handler, this should crash the process
        throw {name: "DB Error", msg: err};
      }
    });
  }
...
Room.prototype.save = function () {
  var self = this;
  db.client.query("UPDATE room_room SET cur_fid = $1, WHERE id = $4",
  [self.cur_fid, self.id],
  function (err, result) {
    log.debug(result);
    if (err) {
      log.error(err);
