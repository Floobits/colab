...
Room.prototype.delete_buf = function (buf_id, agent, cb) {
  ...
  return db.query("UPDATE room_buffer SET deleted = TRUE WHERE fid = $1 AND room_id = $2", [buf.id, self.id], function (err, result) {
    if (err) {
...
exports.add_agent = function (owner, name, agent, user, cb) {
  ...
  db.get_room(owner, name, function (err, result) {
    ...
    try {
      ...
      return cb(util.format("Error in new Room(): %s", e.toString()));
    }
    agent.server.workspaces[db_room.id] = room;

    return db.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      if (err) {
      ...
      if (result.rows.length === 0) {
        var createReadme = function (err) {
          ...
          room.create_buf(settings.readme.name, readme, "utf8", agent, function (err, result) {
        ...
        if (room.repo) {
          return room.repo.update(agent, function (err, result) {
            if (err) {
      ...
      log.log("Loading %s buffers for %s", result.rows.length, room.toString());
