...
Room.prototype.on_solicit = function (req, cb) {
  ...
  this.broadcast(req.name, virtual_agent, null, req);
  return cb();
};
...
ProRoom.prototype.on_solicit = function (req, cb) {
  let data = req.data;
  if (this.outstanding_reqs[req.req_id]) {
    // Ignore it. It's ours.
    log.warn("req %s is outstanding", req.req_id);
    return;
  }
  // TODO: delete these at some point
  this.outstanding_reqs[req.req_id] = req;
  let virtual_agent = new VirtualAgent(this.server.slave.handler, {
    username: data.username,
  });
  // This function is weird and needs the whole req, not just the encapsulated data
  this.solicit(virtual_agent, req.req_id, req, cb);
};
...
ProRoom.prototype.solicit = function (agent, req_id, req, cb) {
  ...
  f.call(s, agent, data, function (err) {
    ...
    case "feedback":
    case "hire":
      actions.broadcast.send_to_master("solicit", req, function (broadcast_err) {
        if (broadcast_err) {
