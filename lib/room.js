...
Room.prototype.rename_buf = function (buf_id, new_path, agent, cb) {
  ...
  return buf.save(false, function (err) {
    self.broadcast("rename_buf", agent, null, {
      id: buf_id,
      old_path: old_path,
      path: buf.path,
      user_id: agent.id,
...
Room.prototype.delete_term = function (agent, id) {
  ...
  self.broadcast("delete_term", agent, null, {
    id: id,
    user_id: agent.id,
    username: agent.username
  });
...
Room.prototype.on_datamsg = function (agent, msg) {
  ...
  if (_.size(msg.to) === 0) {
    self.broadcast("datamsg", agent, null, data);
    return;
  }
...
Room.prototype.part = function (agent) {
  ...
  self.broadcast("part", agent, null, {"user_id": agent.id, "username": agent.username});
