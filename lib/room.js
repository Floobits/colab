...
let Room = function (id, atts, server) {
  ...
  actions.broadcast.onSOLICIT(self.on_solicit, self);
...
Room.prototype.set_atts = function (atts) {
  ...
  if (atts.repo_info && !_.isEmpty(atts.repo_info)) {
    self.repo = new Repo(self, atts.repo_info, atts.private_github && atts.private_git_url);
  }
};
...
Room.prototype.on_solicit = function (req, cb) {
  let data = req.data;
  if (data.path !== util.format("%s/%s", this.owner, this.name)) {
    log.error("wrong path. %s !== %s", data.path, this.path);
    return cb();
  }
  let virtual_agent = new VirtualAgent(this.server.slave.handler, {
    username: data.username,
  });
  this.broadcast(data.name, virtual_agent, null, data);
  return cb();
};
...
Room.prototype.solicit = function (agent, req_id, req) {
  ...
  let auto = {};
  auto.local = function (cb) {
    actions.broadcast.solicit(req, cb);
  };
  auto.broadcast = function (cb) {
    actions.broadcast.send_to_master("solicit", req, cb);
  };
  async.auto(auto, function (err) {
    if (err) {
...
ProRoom.prototype.on_solicit = function (data, cb) {
  let virtual_agent = new VirtualAgent(this.server.slave.handler, {
    username: data.username,
  });
  this.solicit(virtual_agent, data.req_id, data, cb);
};
...
ProRoom.prototype.solicit = function (agent, req_id, req, cb) {
  ...
  f.call(s, agent, data, function (err) {
    var broadcast;
    if (err) {
      return cb(err);
    }
    ...
    return cb(null, broadcast_data, broadcast);
  });
};
