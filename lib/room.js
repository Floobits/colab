  ...
  finish = function (err) {
    ...
    if (_.size(self.bufs) === 0) {
      var createReadme = function (err) {
        ...
        self.create_buf(settings.readme.name, readme, "utf8", agent, null, function (err, result) {
      ...
      if (self.repo) {
        ...
        return self.repo.update(null, agent, function (err, result) {
          if (err) {
...
Room.prototype.broadcast = function (name, req_id, source, data) {
  var self = this,
...
Room.prototype.create_buf = function (path, text, encoding, agent, req_id, cb) {
  var self = this,
    buf,
    fid;
  ...
  self.broadcast("create_buf", req_id, agent, buf.to_json(agent));
  self.dirtify();
...
Room.prototype.delete_buf = function (buf_id, agent, unlink, cb) {
  ...
  self.db.put(buf.db_key, {
    ...
    self.broadcast("delete_buf", null, agent, {
      id: buf_id,
      user_id: agent.id,
...
Room.prototype.rename_buf = function (buf_id, new_path, agent, cb) {
  ...
  return buf.save(false, function (err) {
    self.broadcast("rename_buf", null, agent, {
      id: buf_id,
      old_path: old_path,
      path: buf.path,
      user_id: agent.id,
...
Room.prototype.save = function (cb) {
  ...
  self.server.db.put(util.format("version_%s", self.id), self.version, function (err) {
    ...
    db.query("UPDATE room_room SET cur_fid = $1, updated_at = $2, repo_info = $3 WHERE id = $4",
      [self.cur_fid, now, JSON.stringify(repo_json), self.id], function (err, result) {
        if (err) {
          log.error("Error saving %s: %s", self.toString(), err);
        } else {
...
Room.prototype.create_term = function (agent, name, size, term_id, cb) {
  ...
  self.broadcast("create_term", null, agent, term.to_json());
  return cb(null, term);
};
...
Room.prototype.delete_term = function (agent, id) {
  ...
  self.broadcast("delete_term", null, agent, {
    id: id,
    user_id: agent.id,
    username: agent.username
  });
...
Room.prototype.on_msg = function (agent, req_id, msg_string) {
  ...
  self.broadcast("msg", req_id, agent, msg.to_json());
  self.msgs.push(msg);
...
Room.prototype.on_datamsg = function (agent, msg) {
  ...
  if (_.size(msg.to) === 0) {
    self.broadcast("datamsg", null, agent, data);
    return;
  }
...
Room.prototype.part = function (agent) {
  ...
  self.broadcast("part", null, agent, {"user_id": agent.id, "username": agent.username});
...
Room.prototype.set_temp_data = function (agent, data) {
  ...
  self.broadcast("set_temp_data", null, agent, {
    data: data,
    user_id: agent.id,
    username: agent.username
  });
...
Room.prototype.delete_temp_data = function (agent, data) {
  ...
  self.brodcast("delete_temp_data", agent, "delete_temp_data", {
    data: data,
    user_id: agent.id,
    username: agent.username
  });
