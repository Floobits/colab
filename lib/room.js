...
let ProRoom = function () {
  ...
  actions.broadcast.onSOLICIT(function (data, cb) {
    ...
    self.solicit(virtual_agent, data.req_id, data.data, cb);
  });
};
...
ProRoom.prototype.solicit = function (agent, req_id, req, cb) {
  ...
  if (req.action === "create") {
    if (s) {
      return cb(util.format("A solicitation with ID %s already exists.", req.id));
    }
    s = new solicit.Solicitation(agent, req);
  }

  if (!s) {
    return cb(util.format("No solicitation with ID %s.", req.id));
  }
  ...
  f.call(s, agent, req, function (err) {
    ...
    return cb(null, s.to_json(req.action), broadcast);
  });
};
