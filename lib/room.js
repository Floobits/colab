...
Room.prototype.loadEvents = function (cb) {
  ...
  function finish(err) {
    ...
    // Replay messages just like we normally do
    _.each(msgs, function (m) {
      var msg = new MSG(m);
      self.broadcast("msg", null, null, msg.to_json());
      self.msgs.push(msg);
  ...
  self.db.read_events(self.id, function (err, rs) {
    if (err) {
      return finish(err);
    }
    rs.on("close", finish);
...
Room.prototype.broadcast = function (name, source, req_id, data) {
  ...
  self.event_id++;
  self.events.push(new RoomEvent(self.event_id, name, data));
