...
var site_parse = require("./site_parse");
  ...
  ldb.read_events(self.db, self.id, function (read_events_err, rs) {
    ...
    rs.on("data", function (data) {
      ...
      if (_.contains(PERSISTENT_EVENTS, row.name)) {
        ...
        log.debug("loaded persistent event", evt.to_json());
...
Room.prototype.on_msg = function (agent, req_id, msg_string) {
  ...
  _.each(urls, function (msg_url) {
    site_parse.get_url_msg(msg_url, function (err, result) {
      // Hacky but whatever
      let floobot_msg = new MSG({username: "floobot"}, err || result);
      self.broadcast("msg", null, null, floobot_msg.to_json());
    });
  });
};
