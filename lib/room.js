...
Room.prototype.delete_buf = function(buf_id, cb) {
  ...
  return db.client.query("UPDATE room_buffer SET deleted = 1 WHERE id = $1", [buf.id], function (err, result) {
    if (err) {
...
Room.prototype.rename_buf = function(buf_id, new_path, cb) {
  ...
  log.debug("renaming buf", buf.path, "to", new_path);

  //TODO: make sure new_path doesn't conflict
  self.tree_delete_buf(buf);
  buf.path = new_path;
  self.tree_add_buf(buf);

  return buf.save(false, cb);
};
