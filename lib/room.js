...
var Room = function (id, name, owner, atts) {
  ...
  self.repo_info = atts.repo_info;

  if (self.repo_info && !_.isEmpty(self.repo_info)) {
    self.repo = new Repo(self, self.repo_info);
  }
...
Room.prototype.save = function () {
  ...
  [self.cur_fid, now, JSON.stringify(self.repo_info), self.id],
  function (err, result) {
  ...
  finish = function (err) {
    ...
    return db.get_perms(user.id, room.id, function (err, result) {
      ...
      if (user.is_superuser) {
        ...
        // Only superusers can pull repos right now.
        agent.allowed_actions.push("pull_repo");
      } else {
