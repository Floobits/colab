...
Room.prototype.create_buf = function (path, text, agent, cb) {
  ...
  if (bufs_size > self.max_size) {
    // TODO: tell the user how to increase the max size (pay monies)
    return cb(util.format("Sorry, you've hit this room's max size of %s bytes.", self.max_size));
  }
  ...
  readme_buf = self.get_buf_by_path(settings.readme.name);

  try {
    buf = new ColabBuffer(self, fid, path, text, undefined, true);
  } catch (e) {
    log.error(e);
  ...
  if (readme_buf) {
    self.delete_buf(readme_buf.id, agent, function () {});
  }
  return cb(null, buf);
};
...
Room.prototype.create_term = function(agent, name) {
  var self = this,
    term,
    term_id;

  if (!name || name === "") {
    agent.write("error", {"msg": "A name is required when creating a terminal."});
    return;
  }

  if (!name.match(/^[a-zA-Z0-9\-_]+$/)) {
    agent.write("error", {"msg": "Terminal names can only contain letters, numbers, dashes and underscores."});
    return;
  }
  ...
  if (term) {
    agent.write("error", {"msg": "A terminal with this name already exists."});
    return;
  }

  term_id = ++self.cur_term_id;

  term = new ColabTerm(self, term_id, agent, name);
...
exports.add_agent = function (owner, name, agent, user, cb) {
  ...
  finish = function (err) {
    ...
    return perms.for_room(user.id, room.id, user.is_superuser, function (err, perms) {
      if (err) {
        log.error(err);
        return cb();
      }

      agent.allowed_actions = perms;
  ...
  db.get_room(owner, name, function (err, result) {
    ...
    return db.client.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      ...
      if (result.rows.length === 0) {
        ...
        return room.create_buf(settings.readme.name, settings.readme.text, agent, finish);
      }
      return async.eachLimit(result.rows, 20, function (row, callback) {
        var buf = new ColabBuffer(room, row.fid, row.path, undefined, row.md5, false);
