...
var Room = function (id, name, owner, atts, agent, cb) {
  ...
  db_path = ldb.get_db_path(self.id.toString());
...
Room.prototype.loadBufs = function (agent, cb) {
  ...
  rs = ldb.read_buf_info(self.id, self.db);
  rs.on("close", finish);
  rs.on("error", finish);
...
Room.prototype.save = function (cb) {
  ...
  self.db.put("version", self.version, function (err) {
    if (err) {
      return cb(err);
    }
