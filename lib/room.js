...
Room.prototype.evict = function (reason) {
  ...
  _.each(self.buffers, function (buf, buf_id) {
...
Room.prototype.update_active_users = function () {
  ...
  cache.set(util.format("active_users_%s", self.id), JSON.stringify(agents_json));
    ...
    return db.client.query("SELECT * FROM room_buffer WHERE room_id = $1 AND deleted = FALSE", [db_room.id], function (err, result) {
      ...
      if (result.rows.length === 0) {
        var createReadme = function (err) {
          ...
          room.create_buf(settings.readme.name, settings.readme.text, "utf8", agent, function (err, result) {
