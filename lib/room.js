...
ProRoom.prototype.accept_solicit = function (agent, req_id, req) {
  ...
  try {
    ...
    return agent.error(req_id, e);
  }
  self.broadcast("accept_solicit", agent, req_id, s.to_json());
};

ProRoom.prototype.delete_solicit = function (agent, req_id, req) {
  var self = this,
    s = self.solicitations[req.id];
  ...
  s.delete(agent, req);
  self.broadcast("delete_solicit", agent, req_id, s.to_json());
};

ProRoom.prototype.finish_solicit = function (agent, req_id, req) {
  var self = this,
    s = self.solicitations[req.id];
  ...
  s.finish(agent, req, function (err) {
    if (err) {
      return agent.error(req_id, err);
    }
    self.broadcast("finish_solicit", agent, req_id, s.to_json());
  });
};


exports.Room = Room;
exports.ProRoom = ProRoom;
