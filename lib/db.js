...
function get_perms(user_id, room_id, cb) {
  client.query("SELECT id FROM django_content_type WHERE model = $1", ["room"], function (err, result) {
    ...
    client.query("SELECT codename FROM auth_permission WHERE id in (SELECT permission_id FROM guardian_userobjectpermission WHERE content_type_id = $1 AND user_id = $2 AND object_pk = $3)",
