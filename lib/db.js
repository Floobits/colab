...
var client = new pg.Client(settings.db_info);
...
function auth_user(username, secret, cb) {
  get_user(username, function (err, result) {
    ...
    return client.query("SELECT secret FROM floobits_userprofile WHERE user_id = $1 AND secret = $2",
      [result.id, secret],
      function (err, result) {
        log.debug("auth attempt", err, result);
        if (err) {
          // User not found.
          return cb(err, result);
        }
