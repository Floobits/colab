...
function perms_for_room(user_id, room_id, is_super, cb) {
  ...
  request.get(url, function (error, response, body) {
    ...
    try {
      room_perms = JSON.parse(body).perms;
    } catch (e) {
      return cb(e);
    }

    return cb(null, room_perms);
  });
}

function perms_set(user_id, room_id, room_perms, cb) {
  var options = {
      json: {
        perms: room_perms
      }
...
function user_create(username, cb) {
  var options = {
      json: {
        username: username
      }
