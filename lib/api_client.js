...
var solicitation_create = function (data, cb) {
  var url = util.format("%s/supersecret/contracting/solicitation", settings.django_base_url),
    options = {
      json: data
    };
...
var work_create = function (data, cb) {
  var url = util.format("%s/supersecret/contracting/work", settings.django_base_url),
    options = {
      json: data
    };

  log.debug("Creating work event. URL: %s. JSON: %s", url, JSON.stringify(options.json));
  request.post(url, options, handle_response.bind(null, url, cb));
};

var feedback_create = function (data, cb) {
  var url = util.format("%s/supersecret/contracting/feedback", settings.django_base_url),
    options = {
      json: data
    };
