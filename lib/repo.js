...
Repo.prototype.clone = function (agent, req_id, clone_cb) {
  var self = this,
    url = self.private_github_url || self.url,
    auto;

  if (self.path.indexOf(settings.repos_dir) !== 0) {
    log.error("A SUPER BAD THING ALMOST HAPPENED");
    return clone_cb(util.format("Error cloning %s: Somehow, something broke. Sorry.", self.url));
  }
  auto = {
    rm_dir: function (cb) {
    ...
    clone: ["mk_dir", function (cb) {
      var cmd = util.format("%s '%s' %s", CLONE_CMD[self.type], url, self.path);
