...
var Repo = function (room, repo_info) {
  ...
  if (!_.contains(self.allowed_types, self.type)) {
    throw new Error(util.format("%s is not a valid repo type", self.type));
  }
};
  ...
  utils.walk_dir(self.path, function (err, paths) {
    async.each(_.values(paths), function (filename, callback) {
      ...
      fs.readFile(filename, {encoding: "utf8"}, function (err, text) {
        ...
        function create_buf () {
          self.room.create_buf(rel_path, text, null, function (err, result) {
            if (err) {
        ...
        if (buf) {
          self.room.delete_buf(buf.id, null, create_buf);
        } else {
          create_buf();
        }
