...
Repo.prototype.clone = function (agent, req_id, clone_cb) {
  ...
  let auto = {
    ...
    term: ["rm_dir", function (cb) {
      self.room.create_term(agent, util.format("clone_%s", self.branch), null, null, utils.squelch(cb));
    }],
    clone: ["mk_dir", "term", function (cb, res) {
      ...
      term.broadcast = true;
      // Send something to stdin so ppl in follow mode see the term
      term.stdin(agent, null, "\n");
...
Repo.prototype.pull = function (agent, req_id, cb) {
  ...
  const cp = child_process.exec(cmd, opts, function (err, result) {
    if (err) {
