...
const Repo = function (room, repo_info, private_github_url) {
  ...
  if (!_.includes(ALLOWED_TYPES, self.type)) {
  ...
  if (private_github_url) {
    private_github_url = self.normalize_url(private_github_url);
  }
...
Repo.prototype.to_json = function () {
  const data = {
    type: this.type,
    url: this.url,
    private_github: !!this.private_github_url,
  };
  return data;
};

Repo.prototype.to_private_json = function () {
  const data = this.to_json();
...
Repo.prototype.normalize_url = function (u) {
  ...
  if (!_.includes(ALLOWED_PROTOCOLS, url_obj.protocol)) {
...
Repo.prototype.is_equal = function (req) {
  ...
  const b = this.normalize_branch(req.branch || DEFAULT_BRANCH[req.type]);
  return req.type === this.type && u === this.url && b === this.branch;
};
...
function connect_term (agent, term, cp) {
  ...
  cp.stdout.on("data", function (data) {
    // Hack. Send \r before each line to fix formatting
    term.stdout(null, "DQ==");
  ...
  utils.walk_dir(self.path, function (walk_err, paths) {
    ...
    async.eachLimit(_.values(paths), 20, handle_file, function (err, result) {
      ...
      _.each(self.room.bufs, function (buf, buf_id) {
        if (!_.includes(seen_paths, buf.path)) {
