  ...
  return function (err, status) {
    if (err) {
      if (err.type === "CONNECTION_ERROR") {
        log.error("Memcached connection error: %s", err.description);
        reconnect();
        return;
      }
      try {
      ...
      log.error("Cache error. %s key %s: %s", action, key, err);
      return;
    }
    log.debug("Successfully %s key %s %s", action, key, status || "");
  };
};
...
var cas = function (key, value, cas, opts, cb) {
  ...
  log.log("CAS Setting key %s (cas %s) to %s", key, cas, JSON.stringify(value));
