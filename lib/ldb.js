...
var get_db = function (db, workspace_id, options, cb) {
  ...
  open_dbs[workspace_id] = levelup(db_path, options, function (err, db) {
    if (err) {
      return cb(err);
    }
    log.debug("Opened DB %s.", workspace_id);
    return cb(null, db);
  });
  open_dbs[workspace_id].refcount = 1;
};

var close_db = function (db, workspace_id) {
  db.close(function (err) {
    log.debug("Closed db %s", workspace_id);
    if (err) {
