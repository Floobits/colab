...
function get_db(db, workspace_id, options, cb) {
  ...
  if (db) {
    const db_status = _.get(db, ["db", "status"]);
    log.debug("DB %s status: %s", workspace_id, db_status);
  ...
  db = level(db_path, options, function (err, level_db) {
    if (err) {
      delete open_dbs[workspace_id];
      return cb(err);
    }
...
function close_db(db, workspace_id, cb) {
  log.debug("DB closing %s", workspace_id);
  db.close(function (err) {
    if (err) {
      log.error("Error closing %s: %s", workspace_id, err);
    }
    const open_db = open_dbs[workspace_id];
    if (open_db && open_db.isClosed()) {
      // DB wasn't reopened. Delete it from open_dbs
      delete open_dbs[workspace_id];
    }
    log.debug("DB closed %s", workspace_id);
    if (cb) {
      cb(err);
    }
  });
...
if (settings.log_level === "debug") {
  setInterval(function () {
    log.debug("%s open dbs (max %s)", _.size(open_dbs), settings.max_open_dbs);
    _.each(open_dbs, function (db, id) {
      log.debug("  workspace %s refcount %s status %s", id, db.refcount, _.get(db, ["db", "status"]));
    });
  }, 60000);
}
