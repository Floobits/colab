...
var write = function (maybe_db, workspace_id, encoding, cb) {
  var fin_db = _.once(finish_db);
  get_db(maybe_db, workspace_id, { createIfMissing: true }, function (err, db) {
    var ws;
    if (err) {
      return cb(err);
    }
    if (!_.isFunction(db.createWriteStream)) {
      db = levelws(db);
    }
    ...
    ws.on("error", function (ws_err) {
      log.error("Error writing %s: %s", workspace_id, ws_err);
      fin_db(db, workspace_id);
    });
    return cb(err, ws);
  });
