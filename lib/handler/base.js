...
BaseAgentHandler.prototype.handle = function (msg) {
  ...
  // Everyone can ack
  if (msg.name !== "ack" && !_.includes(self.perms, msg.name)) {
...
BaseAgentHandler.prototype.on_pong = function (req_id) {
  ...
  // Old clients don't send pongs with req_ids. Filter those out.
  this.protocol.server_reqs = _.pickBy(this.protocol.server_reqs, (name) => {
    return name !== "pong";
  });
};
