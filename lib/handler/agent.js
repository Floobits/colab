...
AgentHandler.prototype.auth = function (auth_data) {
  ...
  return self.safe_auto(auto, function (err, result) {
    ...
    // if write fails, eventually destroy gets called and room is set to null
    if (!self.room) {
      log.error("room_info write failed. Bailing from auth.");
      return self.disconnect();
    }
    self.room.broadcast("join", self, null, self.to_json());

    // Replay messages since user joined this workspace
    let replay_event_id = self.room.part_event_ids[self.username] || 0;
...
AgentHandler.prototype.on_perms = function (req_id, req) {
  ...
  api_client.perms_for_room(user.user_id, self.room.id, false, function (err, perms_list) {
    ...
    if (user.is_anon === false) {
      api_client.perms_set(user.user_id, self.room.id, perms_list, function (perms_set_err, updated_perms) {
        if (perms_set_err) {
...
AgentHandler.prototype.on_solicit = function (req_id, req) {
  var self = this;
  return this.room.solicit(this, req_id, req, function (err, result, broadcast) {
    if (err) {
      self.error(req_id, err);
      return;
    }
    if (result) {
      if (!_.isUndefined(broadcast)) {
        self.room.broadcast("solicit", self, req_id, result, broadcast);
        return;
      }
