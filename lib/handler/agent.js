...
AgentHandler.prototype.to_json = function () {
  ...
  return {
    ...
    video_chatting: self.video_chatting,
  };
};
...
AgentHandler.prototype.auth = function (auth_data) {
  ...
  let auto = {
    ...
    get_solicitations: ["room", function (cb, res) {
      // Don't fetch solicitations if user is anon or we're in chat
      if (self.is_anon || !res.room.name) {
        return cb();
      }
      api_client.solicitations_get(res.room.owner, res.room.name, function (err, result) {
        if (err) {
          log.error("Error getting solicitations for %s: %s", self.toString(), err);
          return cb();
        }
    ...
    replay_events: ["room_info", function (cb) {
      ...
      const replay_event_id = self.room.part_event_ids[self.username] || 0;
      _.each(self.room.events, function (evt) {
        if (evt.id > replay_event_id && evt.name === "msg") {
          self.write(evt.name, null, evt.to_json());
        }
      });
      if (self.room && self.room.last_highlight) {
        self.write("highlight", null, self.room.last_highlight);
      }
      cb();
    }],
    replay_solicitations: ["room_info", "get_solicitations", function (cb, res) {
      if (_.isEmpty(res.get_solicitations)) {
        return cb();
      }

      const data = _.reduce(res.get_solicitations, function (s, a) {
      ...
      self.write("solicit", null, {
        "data": solicitation.to_json(),
      });
      if (self.can_contract) {
        // TODO: bubble up errors?
        self.room.solicit(self, null, {
          action: "join",
          data: {
            id: solicitation.id,
          }
          ...
          if (err) {
            log.error("ERROR in join broadcast after solicitation:", err, result);
          }
        });
      }
      return cb();
...
AgentHandler.prototype.on_solicit = function (req_id, req) {
  var self = this;
  return this.room.solicit(this, req_id, req, function (err, result) {
    if (err) {
      self.error(req_id, err);
      return;
    }
