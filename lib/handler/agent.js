...
AgentHandler.prototype.auth = function (auth_data) {
  ...
  if (!_.includes(self.SUPPORTED_VERSIONS, auth_data.version)) {
...
AgentHandler.prototype.on_patch = function (req_id, req) {
  const self = this;
  const buf = self.get_buf_or_error(req_id, req.id);
  if (!buf) {
    return;
  }
...
AgentHandler.prototype.on_saved = function (req_id, req) {
  ...
  // TODO: delay save events slightly? many plugins have pre or post-save hooks
...
AgentHandler.prototype.get_buf_or_error = function (req_id, buf_id) {
  const self = this;
  const buf = self.room.get_buf(buf_id);
  if (!buf) {
    self.error(req_id, util.format("Buffer %s doesn't exist", buf_id));
    return;
  }
  return buf;
};
...
AgentHandler.prototype.on_repo = function (req_id, req) {
  const self = this;
  switch (req.action) {
    case "get":
      self.write("repo", req_id, {
        data: self.room.repo && self.room.repo.to_json(),
      });
      break;
    case "set":
      self.room.set_repo(self, req_id, req.data);
      break;
    case "pull":
      self.room.pull_repo(self, req_id, req);
      break;
    default:
      self.error(req_id, "Invalid repo action.");
  }
};

AgentHandler.prototype.on_pull_repo = function (req_id, req) {
  this.room.pull_repo(this, req_id, req);
};
...
AgentHandler.prototype.on_perms = function (req_id, req) {
  ...
  const user = room.handlers[req.user_id];
  ...
  api_client.perms_for_room((user.is_anon ? "AnonymousUser" : user.username), room.id, false, function (err, perms_list) {
    if (err) {
      log.error(err);
      return;
    }
    ...
    api_client.perms_set(user.username, room.id, perms_list, function (perms_set_err, updated_perms) {
      if (perms_set_err) {
  ...
  room.broadcast("perms", self, null, {
    "action": action,
    "user_id": user.id,
    "perms": fine_grained_perms
  });

  room.broadcast("user_info", self, null, {
    "action": "update",
    "user_id": user.id,
