...
AgentHandler.prototype.auth = function (auth_data) {
  ...
  let auto = {
    ...
    room_info: ["room", function (cb, res) {
      ...
      // TODO: res.room can be null/undefined in rare cases
    ...
    replay_events: ["room_info", function (cb, res) {
      if (!res.room) {
        return cb();
      }
...
AgentHandler.prototype.on_highlight = function (req_id, req) {
  ...
  if (req.ranges.length === 0) {
    return self.error(req_id, util.format("Can't highlight buffer %s: Ranges are empty.", req.id));
  }
