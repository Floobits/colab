...
var MasterHandler = function () {
  ...
  actions.room.onUPDATE(function (workspace_id, workspace) {
    if (!self.server) {
      return;
    }
...
MasterHandler.prototype.destroy = function () {
  ...
  // TODO: remove me once we have zero errors
  this.workspaces_timeout = clearTimeout(this.workspaces_timeout);
};

MasterHandler.prototype.send_stats = function () {
  var self = this;

  self.send_load(function (err) {
    if (err) {
      log.error(err);
    }
  ...
  self.send_workspaces(function (err) {
    ...
    self.workspaces_timeout = setTimeout(self.send_workspaces.bind(self), 10000);
  });
};
