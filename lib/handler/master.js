...
MasterHandler.prototype.on_workspace = function (req_id, data) {
  ...
  if (!workspace_id || !_.isFinite(workspace_id)) {
    self.error(req_id, `No workspace ID or bad ID: ${workspace_id}`);
    return;
  }
  ...
  cb = function (err, result) {
    ...
    if (!self.server) {
      log.error("Master went away. Not sending state back for workspace %s.", workspace_id);
      return;
    }
...
MasterHandler.prototype.on_broadcast = function (req_id, req) {
  ...
  switch (req.action) {
    case "to_user":
      // TODO: so inefficient
      _.each(this.server.handlers, function (h) {
    ...
    case "to_path":
      ...
      from = req.from.split("/");
      virtual_agent = new VirtualAgent({
