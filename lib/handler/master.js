...
MasterHandler.prototype.on_workspace = function (req_id, data, cb) {
  var self = this,
    action = data.action,
    workspace_id = data.id;

  if (!workspace_id || !_.isFinite(workspace_id)) {
    this.error(req_id, "No workspace ID or bad ID");
    return;
  }

  if (!action) {
    this.error(req_id, util.format("No action for workspace %s.", workspace_id));
    return;
  }

  switch (action) {
  case "fetch":
    slave.fetch_workspace(self.server, workspace_id, data.ip, data.port, data.proto, cb);
    break;
  }
};
