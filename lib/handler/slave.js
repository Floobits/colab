...
SlaveHandler.prototype.on_slave = function (req_id, data) {
  ...
  self.https_port = data.https_port;
...
SlaveHandler.prototype.on_workspace = function (req_id, data) {
  ...
  switch (data.action) {
    ...
    case "delete":
      if (self.workspaces[workspace.id]) {
        log.debug("%s: Deleting workspace %s", self.toString(), workspace.id);
      } else {
        log.warn("%s: Deleting workspace %s which doesn't exist!", self.toString(), workspace.id);
      }
    ...
    case "update":
      if (!self.workspaces[workspace.id]) {
        log.warn("%s: Updating workspace %s which doesn't exist!", self.toString(), workspace.id);
      }
      // falls through
    case "fetch":
...
SlaveHandler.prototype.on_error = function (req_id, data) {
  this.ack(req_id);
  if (data) {
    log.error("%s: Error from %s", this.toString(), JSON.stringify(data));
  }
};
