  ...
  case "set":
    ...
    if (!_.isEmpty(self.workspaces)) {
      _.each(data.workspaces, function (w, id) {
        ...
        if (JSON.stringify(server_w) !== JSON.stringify(w)) {
          log.error("workspace mismatch!");
          log.error("%s\n!=%s", JSON.stringify(server_w, null, 2), JSON.stringify(w, null, 2));
        }
      });
    }
    ...
    actions.slave.update_count(self.id, self.workspaces);
  break;
  default:
...
SlaveHandler.prototype.on_workspace = function (req_id, data) {
  ...
  case "update":
    ...
    self.workspaces[workspace.id] = workspace;
    // self.workspaces[workspace.id].version = workspace.version;
    // self.workspaces[workspace.id].active = workspace.active;
    if (workspace.active) {
      self.active_workspaces[workspace.id] = workspace.active;
    }
