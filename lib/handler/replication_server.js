...
var ReplicationServerHandler = function (protocol, auth_timeout_id, workspace_id) {
...
ReplicationServerHandler.prototype.auth = function (server) {
  var self = this;
...
ReplicationServerHandler.prototype.on_room_info = function () {
  var self = this;
};

// Fetch a workspace from another server
// TODO: just pass in the db, not the server
function fetch_workspace(server, workspace_id, proto, ip, port, fetch_cb) {
  var auto = {};

  if (server.workspaces[workspace_id]) {
    // TODO: make it ok to fetch active workspaces
    return fetch_cb(util.format("Workspace %s is active!", workspace_id));
  }
  ...
  auto.get_bufs = ["compare_bufs", function (get_bufs_cb, response) {
    ...
    async.eachLimit(response.compare_bufs, 20, function (buf, cb) {
      ...
      log.debug("Hitting %s", url);
      request.get(url, options, function (err, get_buf_res, body) {
        var buf_md5;

        if (err) {
          return cb(err, get_buf_res);
        }

        if (get_buf_res.statusCode >= 400) {
          return cb(util.format("Code %s from %s. Body: %s", get_buf_res.statusCode, url, body));
        }
  ...
  async.auto(auto, function (err, response) {
    ...
    if (!_.isFinite(response.get_buf_list.version)) {
      ...
      return fetch_cb(util.format("Workspace %s had bad version: %s", workspace_id, response.get_buf_list.version));
    }
    server.db.put(util.format("version_%s", workspace_id), response.get_buf_list.version, function (version_err) {
      if (version_err) {
        log.error("Error updating workspace %s version: %s", workspace_id, version_err.toString());
        return fetch_cb(version_err);
      }
      log.debug("Fetched workspace %s", workspace_id);
      return fetch_cb(null, {
