...
ReplicationServerHandler.prototype.fetch = function (workspace_id, cb) {
  ...
  self.request("replicate", {
    ...
    supported_encodings: ["base64", "utf8"],
...
ReplicationServerHandler.prototype.on_room_info = function (res_id, ri) {
  ...
  auto.get_bufs = ["compare_bufs", function (get_bufs_cb, response) {
    ...
    function save_bufs (buf, cb) {
      ...
      self.request("get_buf", {
        ...
        db.put(util.format("buf_%s", buf.id), {
          ...
          const data_buf = new Buffer(data.buf, data.encoding);
          log.log("db.put %s/%s length %s encoding: %s", workspace_id, buf.id, data.buf.length, data.encoding);
          if (data.encoding === "base64") {
            if (data.buf.length > 8 && data_buf.length < data.buf.length / 2) {
              let err = util.format("Invalid base64 for %s/%s %s", workspace_id, data.id, data.path, data.buf.slice(0, 100));
              log.error(err);
              return cb(err, data);
            }
          }
    ...
    async.eachSeries(response.compare_bufs, save_bufs, get_bufs_cb);
  }];
