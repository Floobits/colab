...
var fetch_workspace = function (server, req, res) {
  ...
  // TODO: compare md5s and only fetch ones that differ

  auto.get_bufs = ["get_buf_list", "create_write_stream", function (cb, response) {
    var ws = response.create_write_stream;
    ws.on("error", function (err) {
      // TODO: don't be stupid here
      return res.send(500, err.toString());
    });
    async.eachLimit(_.values(response.get_buf_list.bufs), 20, function (buf, cb) {
