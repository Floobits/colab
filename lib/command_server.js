...
var fetch_workspace = function (server, req, res) {
  ...
  auto.get_bufs = ["compare_bufs", function (cb, response) {
    ...
    async.eachLimit(response.compare_bufs, 20, function (buf, cb) {
      ...
      // TODO: generate a cert for internal usage
      options = {
        json: true,
        strictSSL: false
      };
  ...
  async.auto(auto, function (err, result) {
    ...
    if (!result.get_buf_list.version) {
      log.error("Workspace %s had no version! Result: %s", workspace_id, result);
      return res.send(500, util.format("Workspace %s had no version! Result: %s", workspace_id, result));
    }
