...
BaseAgentConnection.prototype.on_request_perms = function (req) {
  ...
  if (self.request_perms_timeout > now) {
    return user.error(util.format("You can not make another request for %s seconds.", (timeout - now) / 1000));
  }
  ...
  timeout = Math.min(Math.pow(1.5, self.bad_perm_requests), 60 * 60);
