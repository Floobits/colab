...
var BaseAgentConnection = function (id, conn, server) {
  ...
  // TODO: no need to stick this on self
...
BaseAgentConnection.prototype.disconnect = function (reason, cb) {
  ...
  utils.set_state(self, self.CONN_STATES.DISCONNECTING);
...
BaseAgentConnection.prototype.destroy = function () {
  ...
  utils.set_state(self, self.CONN_STATES.DESTROYED);
  ...
  if (_.size(self.outstanding_reqs) > 0) {
    log.log("%s outstanding reqs for destroyed client %s:", _.size(self.outstanding_reqs), self.toString());
    _.each(self.outstanding_reqs, function (req, req_id) {
      log.debug("%s: %s", req_id, JSON.stringify(req));
    });
  }
};
