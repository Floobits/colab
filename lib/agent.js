...
BaseAgentConnection.prototype.auth = function (auth_data, cb) {
  ...
  return async.auto(auto, function (err, result) {
    ...
    if (self.is_anon === false) {
      user_rooms = _.filter(self.server.agents, function (agent, k) {
      ...
      cache.set(util.format("now_editing_user_%s", self.user_id), JSON.stringify(user_rooms));
    }
