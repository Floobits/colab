...
BaseAgentConnection.prototype.disconnect = function (reason, cb) {
  ...
  log.debug("disconnecting client", self.id, "ip", self.remote_address);
  if (reason) {
  ...
  try {
    ...
    log.error("Couldn't update active workspaces for client", self.id, ": ", e2);
  }
...
BaseAgentConnection.prototype.auth = function (auth_data, cb) {
  ...
  if (!OK) {
    log.error("Bad auth JSON; disconnecting client.");
    return self.disconnect();
  }
  auth_data.version = Number(auth_data.version);
  if (!_.contains(SUPPORTED_VERSIONS, auth_data.version)) {
    log.error(util.format("Unsupported client version: %s; disconnecting client.", auth_data.version));
    return self.disconnect();
  }
  ...
  if (self.client === "flootty" && self.version < 0.1) {
    return self.disconnect("You need to update flootty to version 1.10 or greater. Please run 'pip install --upgrade flootty'");
  }
...
BaseAgentConnection.prototype.update_active_workspaces = function () {
  var self = this,
    workspaces;
  ...
  workspaces = _.filter(self.server.agents, function (agent, k) {
    return agent.id === self.id && agent.parted === false;
  }).map(function (agent) {
    var a = agent.to_json();
    delete a.perms;
    a.id = agent.id;
    if (_.isObject(agent.room)) {
      a.room_id = agent.room.id;
    }
    return a;
  });

  cache.set(util.format("now_editing_user_%s", self.user_id), JSON.stringify(workspaces));
};
...
BaseAgentConnection.prototype.on_set_buf = function (req) {
  ...
  // TODO: this could falsely trigger if someone's typing in a buffer that another user deletes
  if (buf) {
    return buf.set(self, req.buf, req.md5, req.encoding, function (err, result) {
      if (err) {
        self.error(err);
      }
    });
...
BaseAgentConnection.prototype.on_kick = function (req) {
  var self = this,
    agent;
  if (req === undefined || !_.isFinite(req.id)) {
    self.error("You tried to kick someone, but you didn't specify the id.");
    return;
  }
  agent = self.room.agents[req.id];
  if (agent) {
    agent.disconnect("Kicked by " + self.username);
  } else {
    self.error("user id " + req.id + " doesn't exist");
  }
};
...
BaseAgentConnection.prototype.on_perms = function (req) {
  ...
  perms.for_room(user.id, self.room.id, false, function (err, perms_list) {
    if (err) {
      log.error(err);
      return;
    }
    ...
    if (user.is_anon === false) {
      perms.set(user.id, self.room.id, perms_list, function (err, updated_perms) {
        if (err) {
...
AgentConnection.prototype.on_data = function (d) {
  ...
  if (self.buf.length > settings.max_buf_len * (4 / 3)) {
...
var SIOAgentConnection = function (id, conn, server) {
  ...
  conn.on("create_user", function (data) {
...
SIOAgentConnection.prototype.remove_listeners = function () {
  var self = this;
...
SIOAgentConnection.prototype._remote_address = function () {
  var self = this;
  try {
