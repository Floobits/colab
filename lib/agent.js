...
var BROADCAST_ACTIONS = ['create_buf'];
...
var BaseAgentConnection = function (id, conn, server) {
  ...
  self.allowed_actions = ["patch", "get_buf", "create_buf", "highlight"];
};
...
AgentConnection.prototype.on_data = function (d) {
  ...
  try {
    ...
    log.error("couldn't parse json", msg, "exception:", e);
    self.disconnect();
  }
