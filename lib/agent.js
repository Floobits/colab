...
BaseAgentConnection.prototype.disconnect = function (reason, cb) {
  ...
  if (self.state === CONN_STATES.SENT_DISCONNECT) {
    return cb && cb();
  }
  self.state = CONN_STATES.SENT_DISCONNECT;

  log.log("Disconnecting client %s ip %s. Reason: %s", self.toString(), self.remote_address, reason);
  if (reason) {
    self.write("disconnect", {reason: reason}, self.destroy.bind(self));
  } else {
    self.destroy();
  }
  ...
  try {
    ...
    log.error("Couldn't update active workspaces for client %s: %s", self.toString(), e2);
  }
...
AgentConnection.prototype.disconnect = function (reason, cb) {
  ...
  try {
    ...
    log.error("Couldn't destroy connection: %s: %s", self.id, e);
  }
...
AgentConnection.prototype.write = function (name, json, cb) {
  ...
  if (self.state === CONN_STATES.SENT_DISCONNECT) {
    log.error("Somebody called write after we sent a disconnect!");
    return;
  }
  json.name = name;
