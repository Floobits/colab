...
var SUPPORTED_VERSIONS = [0.01, 0.02, 0.03, 0.1];
...
BaseAgentConnection.prototype._send_metrics = function (force) {
  ...
  mixpanel.people.increment(self.user_id, data);
};
...
BaseAgentConnection.prototype.allow = function (actions) {
  log.debug("Allowed actions:", actions);
  return;
};
...
BaseAgentConnection.prototype.update_active_workspaces = function () {
  var self = this,
    key = util.format("now_editing_user_%s", self.user_id),
    workspaces;
  ...
  workspaces = _.filter(self.server.agents, function (agent) {
    return agent.user_id === self.user_id && agent.parted === false;
  }).map(function (agent) {
    var a = agent.to_json();
    delete a.perms;
  ...
  cache.gets(key, function (err, result) {
    if (err) {
      log.error(err);
      cache.set(key, JSON.stringify(workspaces));
      return;
    }
    ...
    workspaces = _.union(result, workspaces);
    if (_.isUndefined(result[key]) && _.isUndefined(result[key].cas)) {
      log.error("No cas for %s", key);
      cache.set(key, JSON.stringify(workspaces));
      return;
    }
    cache.cas(key, JSON.stringify(workspaces), result[key].cas);
  });
};
...
BaseAgentConnection.prototype.on_patch = function (req) {
  ...
  if (!_.isNull(self.room.readme_buff) && buf.id === self.room.readme_buf.id) {
...
BaseAgentConnection.prototype.on_delete_buf = function (req) {
  var self = this;
  self.room.delete_buf(req.id, self, function (err) {
    if (err) {
