...
BaseAgentConnection.prototype.get_term_or_error = function(term_id, owner_only, cb) {
  ...
  if (!term) {
    return self.write("error", {"msg": util.format("Terminal %s doesn't exist", term_id)});
  }
  if (owner_only && self.id !== term.owner.id) {
    self.write("error", {"msg": util.format("Only the terminal owner can perform this action.")});
    return;
  }
  return cb(term);
};
