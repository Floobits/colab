...
var BaseAgentConnection = function (id, conn, server) {
  ...
  self.state = self.CONN_STATES.AUTH_WAIT;
...
BaseAgentConnection.prototype.CONN_STATES = {
  AUTH_WAIT: 1,
  JOINED: 2,
  DISCONNECTING: 3,
  DESTROYED: 4
...
BaseAgentConnection.prototype.disconnect = function (reason, cb) {
  ...
  self.state = self.CONN_STATES.DISCONNECTING;
...
BaseAgentConnection.prototype.destroy = function () {
  ...
  self.state = self.CONN_STATES.DESTROYED;
...
BaseAgentConnection.prototype.auth = function (auth_data, cb) {
  ...
  auto = {
    ...
    room: ["authed_user", function (cb, res) {
      ...
      self.state = self.CONN_STATES.JOINED;
  ...
  return async.auto(auto, function (err, result) {
    ...
    self.room.broadcast("join", null, self, self.to_json());

    _.each(self.room.msgs, function (msg) {
      self.write("msg", null, msg.to_json());
    });
...
BaseAgentConnection.prototype.on_ping = function (req_id, req) {
  var self = this;
  self.write("pong", req_id, {});
};

BaseAgentConnection.prototype.on_pong = function (req_id, req) {
  var self = this;

  clearTimeout(self.idle_timeout);
  self.idle_timeout = setTimeout(self.disconnect.bind(self), 60000);
  clearTimeout(self.heartbeat);
  self.heartbeat = setTimeout(self.write.bind(self, "ping", null, {}), 15000);
};
...
BaseAgentConnection.prototype.on_patch = function (req_id, req) {
  ...
  buf.patch(req_id, self, req.patch, req.md5_before, req.md5_after);
};
...
BaseAgentConnection.prototype.on_set_buf = function (req_id, req) {
  ...
  buf.set(req_id, self, req.buf, req.md5, req.encoding, false, function (err) {
    if (err) {
...
BaseAgentConnection.prototype.on_get_buf = function (req_id, req) {
  ...
  if (!buf.loaded) {
    buf.once("load", function () {
      buf.send_get_buf(req_id, self, 0);
    });
  } else {
    buf.send_get_buf(req_id, self, 0);
  }
...
BaseAgentConnection.prototype.on_create_buf = function (req_id, req) {
  ...
  self.room.create_buf(req.path, req.buf, req.encoding, self, req_id, function (err) {
    if (err) {
...
BaseAgentConnection.prototype.on_highlight = function (req_id, req) {
  ...
  if (buf) {
    ...
    self.error(req_id, util.format("Can't highlight buffer %s. It doesn't exist", req.id));
  }
};
...
BaseAgentConnection.prototype.on_saved = function (req_id, req) {
  ...
  self.room.broadcast("saved", req_id, self, {
    id: req.id,
    user_id: self.id
  });
};
...
BaseAgentConnection.prototype.on_pull_repo = function (req_id, req) {
  ...
  if (self.room.repo && _.isEqual(req, self.room.repo.to_json())) {
    self.room.repo.update(req_id, self, cb);
  } else {
    try {
    ...
    self.room.repo.clone(req_id, self, cb);
  }
};
...
BaseAgentConnection.prototype.on_perms = function (req_id, req) {
  ...
  self.room.broadcast("perms", null, self, {
    "action": action,
    "user_id": user.id,
    "perms": fine_grained_perms
  });

  self.room.broadcast("user_info", null, self, {
    "action": "update",
    "user_id": user.id,
...
BaseAgentConnection.prototype.disconnect_unauthed_client = function () {
  var self = this;
  if (self.state > self.CONN_STATES.AUTH_WAIT) {
...
BaseAgentConnection.prototype.ack = function (req_id) {
  ...
  // TODO: check if we've already acked this req_id
  ...
  try {
    ...
    if (_.has(msg, "req_id")) {
      if (msg.req_id % 1 === 0 && msg.req_id > self.cur_req_id) {
        ...
        self.outstanding_reqs[req_id] = msg;
      } else {
