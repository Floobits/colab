...
BaseAgentConnection.prototype.on_delete_temp_data = function(req) {
  ...
  _.each(req.data, function (k) {
    delete self.room.temp_data[k];
  });
  self.room.emit("dmp", self, "delete_temp_data", {
    data: req.data,
    user_id: self.id,
...
BaseAgentConnection.prototype.on_set_temp_data = function(req) {
  ...
  _.each(req.data, function (v, k) {
    log.debug("comparing key", k, ":", self.room.temp_data[k], v);
    if (!_.isEqual(self.room.temp_data[k], v)) {
      changed = true;
  ...
  _.extend(self.room.temp_data, req.data);

  self.room.emit("dmp", self, "set_temp_data", {
    data: req.data,
    user_id: self.id,
    username: self.username
  });
