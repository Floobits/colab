...
var BROADCAST_ACTIONS = ["create_buf", "delete_buf", "rename_buf", "create_term", "delete_term"];
...
BaseAgentConnection.prototype.on_create_term = function(req) {
  var self = this;
  self.room.create_term(self, req.name);
};
...
BaseAgentConnection.prototype.on_term_stdin = function(req) {
  ...
  term = self.room.get_term(req.id);
  if (term) {
    term.stdin(self, req.data);
  } else {
    self.write("error", {"msg": util.format("Can't send to terminal %s. It doesn't exist", req.id)});
  }
};
...
BaseAgentConnection.prototype.on_term_stdout = function(req) {
  ...
  term = self.room.get_term(req.id);
  if (term) {
    if (self.id !== term.owner.id) {
      self.write("error", {"msg": util.format("Only the terminal owner can send stdout. You should never see this error.")});
      return;
    }
