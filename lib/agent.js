...
var BaseAgentConnection = function (id, conn, server) {
  ...
  self.allowed_actions = utils.db_perms_mapping[2];
};
...
BaseAgentConnection.prototype.on_create_buf = function (req) {
  var self = this;
  var buf = self.room.create_buf(req.path, null, req.buf);
  if (buf) {
...
BaseAgentConnection.prototype.on_kick = function (req) {
  ...
  if (req === undefined || !_.isFinite(req.user_id)) {
    self.disconnect("You tried to kick someone, but the user_id wasn't specified.");
    return;
  }
  ...
  if (agent) {
    ...
    self.write("error", {"msg": "user id doesn't exist"});
  }
};
