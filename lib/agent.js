...
BaseAgentConnection.prototype.on_pull_repo = function (req) {
  ...
  if (!self.room.repo || _.isEqual(req.repo_info, self.room.repo_info) === false) {
    self.room.repo_info = req.repo_info;
    self.room.repo = new Repo(self, self.room.repo_info);
  }
  self.room.repo.pull(function (err, result) {
    if (err) {
