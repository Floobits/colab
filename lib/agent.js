...
BaseAgentConnection.prototype.on_get_buf = function (req) {
  var self = this,
    ...
    buf_json;
  if (!req.id) {
    self.write("error", {"msg": "update yo shit!", "flash": true});
    return;
  }
  if (!buf.loaded) {
...
BaseAgentConnection.prototype.on_create_buf = function (req) {
  ...
  self.room.create_buf(req.path, req.buf, self, cb);
};
...
BaseAgentConnection.prototype.on_delete_buf = function (req) {
  ...
  self.room.delete_buf(req.id, self, function (err, buf) {
    if (err) {
      self.write("error", {"msg": util.format("Error deleting buffer %s: %s", req.id, err)});
    }
  });
};
...
BaseAgentConnection.prototype.on_rename_buf = function (req) {
  ...
  self.room.rename_buf(req.id, req.path, self, function (err, old_path) {
    if (err) {
      ...
      self.write("error", {"msg": util.format("Error renaming buffer %s to %s: %s", req.id, req.path, err), "flash": true});
    }
  });
};
...
BaseAgentConnection.prototype.on_highlight = function (req) {
  ...
  if (buf) {
    ...
    self.write("error", {"msg": util.format("Can't highlight buffer %s. It doesn't exist", req.id)});
  }
};
...
BaseAgentConnection.prototype.on_delete_temp_info = function(req) {
  var self = this,
    msg;

  // TODO: validate and stuff
  _.each(req.data, function (k) {
    delete self.room.temp_info[k];
  });
  self.room.emit("dmp", self, "delete_temp_info", {
    data: req.data,
    user_id: self.id,
...
BaseAgentConnection.prototype.on_set_temp_info = function(req) {
  var self = this,
    changed = false,
    msg,
    temp_info,
  ...
  // TODO: validate and stuff
  if (_.keys(req.data).length > 1) {
    log.debug("too many keys");
    return;
  }
  if (!req.data.hangout || !req.data.hangout.url) {
    log.debug("no hangout url key");
    return;
  }
  url_obj = url.parse(req.data.hangout.url);
  if (url_obj.protocol !== "https:" || !url_obj.hostname.match(/\.google\.com$/)) {
    log.debug("hangout url does not match");
    return;
  }

  _.each(req.data, function (v, k) {
    log.debug("comparing key", k, ":", self.room.temp_info[k], v);
    if (!_.isEqual(self.room.temp_info[k], v)) {
      changed = true;
  ...
  _.extend(self.room.temp_info, req.data);

  self.room.emit("dmp", self, "set_temp_info", {
    data: req.data,
    user_id: self.id,
    username: self.username
  });
...
BaseAgentConnection.prototype.on_pull_repo = function (req) {
  ...
  function cb (err, result) {
    ...
    if (err) {
      return self.write("error", {"msg": "Error pulling repository:" + err, "flash": true});
    }
    self.room.save();
  }
  ...
  if (self.room.repo && _.isEqual(req, self.room.repo.to_json())) {
    ...
    try {
      ...
      self.write("error", {"msg": "Error creating repository:" + e.toString()});
      return;
    }
...
var AgentConnection = function (id, conn, server) {
  ...
  self.on("delete_temp_info", self.on_delete_temp_info.bind(self));
  self.on("set_temp_info", self.on_set_temp_info.bind(self));
