...
const CPUS = _.size(os.cpus());
// Total memory in megabytes
const TOTAL_MEM = os.totalmem() / Math.pow(2, 20);
...
function get_load(cb) {
  ...
  l.heap = _.mapValues(l.heap, function (v) {
  ...
  child_process.exec(util.format("df -P -m %s", settings.base_dir), function (err, stdout) {
    ...
    // Disk used doesn't count deleted files that are still open. We really care about total - available.
    l.disk.usage = (l.disk.total - l.disk.available) / l.disk.total;
    return cb(err, l);
  });
}
...
function all_workspaces(server, cb) {
  ...
  if (!server) {
    return cb("No server handle");
  }

  rs = server.db.createReadStream({
    gte: "version_",
    lte: "version_999999999999999"
  });
  ...
  rs.on("data", function (data) {
    let workspace_id = parseInt(data.key.slice(8), 10);
    ...
    let workspace = server.workspaces[workspace_id];
    if (workspace) {
